...
extern inline void rdtsc(unsigned long *time) {
...
void start_receiver(void) {
  ...
  printf("calling l4_task_new(%x.%x, %d, %p, %p, %x.%x);\n",
         receiver_id.lh.high, receiver_id.lh.low, 255, &receiver_stack[255],
         pong, l4_myself().lh.high, l4_myself().lh.low);
  receiver_id =
      l4_task_new(receiver_id, 255, (unsigned int)&receiver_stack[255],
  ...
  if (l4_i386_ipc_send(
          receiver_id, L4_IPC_SHORT_FPAGE, 0, /* send base */
          l4_fpage(0, L4_WHOLE_ADDRESS_SPACE, L4_FPAGE_RW, L4_FPAGE_MAP).fpage,
  ...
  if (l4_i386_ipc_call(receiver_id, L4_IPC_SHORT_MSG, dummy, dummy,
...
#define ROUNDS 100000
    ...
    rdpmc(0, &cnt1);
    printf("\ncycles: %ld (%lx, %lx)\n",
           (ping_stop - ping_start) / (ROUNDS * 2), ping_stop, ping_start);
    printf("rdpmc(0) = %Ld\n", cnt0);
    printf("rdpmc(0) = %Ld\n", cnt1);
