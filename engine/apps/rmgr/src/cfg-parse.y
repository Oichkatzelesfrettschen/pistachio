...
#define SET_DEBUG_OPT(proto, action)                                           \
  do {                                                                         \
    ...
    if (action)                                                                \
      debug_log_types = (2L | (action << 16) | (debug_log_types & 7));         \
  } while (0);
  %
}
...
% union {
  ...
  struct {
    ...
    % token TASK MODNAME CHILD IRQ MAX IN MASK MEMORY HIMEM LOGMCP BOOTMCP
          BOOTPRIO %
    ...
    token MEM_PROTO MEM_FREE MEM_FREE_FP % token IRQ_PROTO IRQ_GET IRQ_FREE %
    token RMGR_PROTO RMGR_PING % token LOG_IT KDEBUG %
    token<string> UNSIGNED STRING % type<number> number %
...
rule : taskspec constraints modules {
  if (ctask) {
    ...
    printf("RMGR: configured task 0x%x: [ m:%x,%x,%x hm:%x,%x,%x\n"
           "      t:%x,%x,%x i:%x lmcp:%x s:%x,%x,%x\n"
           "      mcp:%x prio:%x small:%x ]\n",
           ctask, q->mem.low, q->mem.high, q->mem.max, q->himem.low,
           q->himem.high, q->himem.max, q->task.low, q->task.high, q->task.max,
           q->irq.max, q->log_mcp, q->small.low, q->small.high, q->small.max,
...
numerical_options : number number {
  debug_log_mask = $1;
  debug_log_types = $2;
};
...
verbose_options : verbose_option verbose_options | verbose_option;

log_options : numerical_options | verbose_options;
...
| TASK taskname | TASK RMGR { ctask = __cfg_task = myself.id.task; }
...
taskname : MODNAME string {
  ...
  for (i = first_task_module; i < mb_info.mods_count; i++) {
    ...
    if (strstr(n, $2)) {
      ctask = __cfg_task = myself.id.task + 1 + (i - first_task_module);
      assert_mod = i;
      goto taskname_end;
    }
  }
  ...
  printf("RMGR: WARNING: couldn't find modname %s, "
         ...
         $2);
  strncpy(mb_mod_names[first_not_bmod_free_modnr], $2, MOD_NAME_MAX);
...
| IRQ irqconstraints {
  unsigned mask = 0, i;
  for (i = max(0, c_low); i <= min(15, c_max); i++) {
    mask |= 1L << i;
  }
  quota[ctask].irq.max &= c_mask & mask;
  c_low = 0;
  c_high = -1;
  c_max = -1;
  c_mask = 0xffffffff;
...
irqconstraints : numconstraints | maskconstraints;

numconstraints : numconstraint numconstraints | numconstraint;

numconstraint : MAX number { c_max = min(c_max, $2); }
| IN setspec {
  c_low = max(c_low, $2.low);
