...
void pager(void) {
  ...
  for (;;) {
    ...
    while (!err) {
      ...
      } else /* valid requester */
      {
        ...
        if (d1 == 0xfffffffc) {
          ...
          d1 = 0;
          d2 = l4_fpage((dword_t)l4_info, L4_LOG2_PAGESIZE, L4_FPAGE_RO,
                        L4_FPAGE_MAP)
                   .fpage;
          desc = L4_IPC_SHORT_FPAGE;
          ...
          if ((d1 & 1) && (d2 == (L4_LOG2_SUPERPAGESIZE << 2))) {
            ...
            if (!no_pentium && memmap_alloc_superpage(pfa, t.id.task)) {
              d1 &= L4_SUPERPAGEMASK;
              d2 =
                  l4_fpage(d1, L4_LOG2_SUPERPAGESIZE, L4_FPAGE_RW, L4_FPAGE_MAP)
                      .fpage;
              desc = L4_IPC_SHORT_FPAGE;
          ...
          if (memmap_alloc_page(d1, t.id.task)) {
            d1 &= L4_PAGEMASK;
            d2 =
                l4_fpage(d1, L4_LOG2_PAGESIZE, L4_FPAGE_RW, L4_FPAGE_MAP).fpage;
            desc = L4_IPC_SHORT_FPAGE;
          ...
          if (memmap_alloc_superpage(pfa + 0x40000000, t.id.task)) {
            ...
            l4_i386_ipc_call(my_pager, L4_IPC_SHORT_MSG, pfa, 0,
                             L4_IPC_MAPMSG(scratch, L4_LOG2_SUPERPAGESIZE), &d1,
                             &d2, L4_IPC_NEVER, &result);

            /* grant the superpage to the subtask */
            d1 = pfa;
            d2 = l4_fpage(scratch, L4_LOG2_SUPERPAGESIZE, L4_FPAGE_RW,
                          L4_FPAGE_GRANT)
                     .fpage;
            desc = L4_IPC_SHORT_FPAGE;
          }
    ...
    reply:
      ...
      err = l4_i386_ipc_reply_and_wait(t, desc, d1, d2, &t, 0, &d1, &d2,
                                       L4_IPC_TIMEOUT(0, 1, 0, 0, 0, 0),
                                       /* snd timeout = 0 */
                                       &result);

      /* send error while granting?  flush fpage! */
      if (err == L4_IPC_SETIMEOUT && desc == L4_IPC_SHORT_FPAGE && (d2 & 1)) {
...
static void find_free(dword_t *d1, dword_t *d2, owner_t owner) {
  ...
  for (address = 0; address < mem_high; address += L4_SUPERPAGESIZE) {
    ...
    for (; address < mem_high; address += L4_PAGESIZE) {
      ...
      *d1 = address;
      *d2 =
          l4_fpage(address, L4_LOG2_PAGESIZE, L4_FPAGE_RW, L4_FPAGE_MAP).fpage;
...
#define ROUND_PAGE(x) (((x) + L4_PAGESIZE - 1) & L4_PAGEMASK)
  ...
  for (bor = 0, eor = 0; eor <= mem_high; eor += L4_PAGESIZE) {
    if ((memmap_owner_page(eor) != owner) || (eor >= mem_high) ||
        (eor == 0x00001000) || (eor == 0x00002000) || (eor == eor_reserved0) ||
        (eor == eor_sigma0)) {
      if (bor != eor) {
        switch (owner) {
        case O_RESERVED:
