...
static int alloc_exec_read_exec(void *handle, vm_offset_t file_ofs,
                                vm_size_t file_size, vm_offset_t mem_addr,
                                vm_size_t mem_size,
                                exec_sectype_t section_type);
static int alloc_exec_read(void *handle, vm_offset_t file_ofs, void *buf,
...
static void boot_panic(const char *fmt, ...) {
  for (;;) {
    char c;
    va_list args;
    va_start(args, fmt);
...
void init(void) {
  ...
  for (address = (vm_offset_t)&_start & L4_PAGEMASK;
       address < ((vm_offset_t)&_stext & L4_PAGEMASK); address += L4_PAGESIZE) {
...
static void init_globals(void) {
  ...
  l4_thread_ex_regs(myself, (dword_t)-1, (dword_t)-1, &my_preempter, &my_pager,
                    &dummy, &dummy, &dummy);
...
static void init_iospace(void) {
  ...
  /* the address of the receive window doesn't matter actually, but should not
   * be 0 */
  foo_fp = l4_fpage((int)0xF0000000, 0, 1, 0);

  dw0 = 0xF0000040;

  l4_i386_ipc_call(my_pager, L4_IPC_SHORT_MSG, dw0, dw0, (void *)foo_fp.fpage,
                   &dw0, &dw0, L4_IPC_NEVER, &res);
}
#endif
...
static void init_irq(void) {
  ...
  for (i = 0; i < IRQ_MAX; i++) {
    ...
    *--sp = 0; /* faked return address */
    ...
    pager = my_pager;
    l4_thread_ex_regs(t, (dword_t)__irq_thread, (dword_t)sp, &preempter, &pager,
                      &dummy, &dummy, &dummy);

    /* handshake and status check */
    err = l4_i386_ipc_receive(t, L4_IPC_SHORT_MSG, &code, &dummy, L4_IPC_NEVER,
                              &result);
...
static void init_task(void) {
  ...
  first_not_bmod_free_task = first_not_bmod_task =
      myself.id.task + 1 + (mb_info.mods_count - first_task_module);
}

static void init_small(void) { small_init(); }
...
static void init_memmap(void) {
  ...
  if (!no_pentium) {
    ...
    for (address = L4_SUPERPAGESIZE; address < MEM_MAX;
      ...
      error = l4_i386_ipc_call(
          my_pager, L4_IPC_SHORT_MSG, address | 1, L4_LOG2_SUPERPAGESIZE << 2,
          L4_IPC_MAPMSG(0, L4_WHOLE_ADDRESS_SPACE), &sfpage.snd_base,
          &sfpage.fpage.fpage, L4_IPC_NEVER, &result);
  ...
  error = l4_i386_ipc_call(my_pager, L4_IPC_SHORT_MSG, 0, 0,
                           L4_IPC_MAPMSG(0, L4_WHOLE_ADDRESS_SPACE),
                           &sfpage.snd_base, &sfpage.fpage.fpage, L4_IPC_NEVER,
                           &result);
  /* XXX should i check for errors? */
  ...
  /* now add all memory we can get from L4 to the memory map as free
     memory */
  for (;;) {
    error = l4_i386_ipc_call(my_pager, L4_IPC_SHORT_MSG, 0xfffffffc, 0,
                             L4_IPC_MAPMSG(0, L4_WHOLE_ADDRESS_SPACE),
  ...
  for (address = mod_range_start & L4_PAGEMASK; address <= mod_range_end;
       address += L4_PAGESIZE) {
    /* check if we really can touch the memory */
    if (address > mem_high) {
      printf("RMGR: Can't touch memory at %08x for boot modules. ", address);
      if (address > mem_phys)
  ...
  /* page in adapter space explicitly.  the adapter space will be
     marked "reserved" and special-cased in the pager() in memmap.c */
  for (address = mem_lower * 1024L; address < 0x100000L;
       address += L4_PAGESIZE) {
    error = l4_i386_ipc_call(my_pager, L4_IPC_SHORT_MSG, address, 0,
                             L4_IPC_MAPMSG(0, L4_WHOLE_ADDRESS_SPACE),
                             &sfpage.snd_base, &sfpage.fpage.fpage,
                             L4_IPC_NEVER, &result);
    /* XXX should i check for errors? */
  ...
  for (address = ((vm_offset_t)(&_start)) & L4_PAGEMASK;
       address <= (vm_offset_t)&_end; address += L4_PAGESIZE) {
  ...
  if (fiasco_symbols) {
    ...
    if (!address) {
      ...
      asm("int $3 ; cmpb $30,%%al"
          :
          : "a"(fiasco_symbols) /* offset symbols */, "b"(0) /* task number */,
  ...
  error = l4_i386_ipc_call(my_pager, L4_IPC_SHORT_MSG, 1, 1,
  ...
  printf("RMGR: total RAM size = %6ld KB (reported by bootloader)\n"
         ...
         mb_info.mem_upper + mb_info.mem_lower, free_size / 1024L,
         reserved_size / 1024L);
...
static void start_tasks(void) {
  ...
  for (i = first_task_module; i < last_task_module; i++, i2++) {
    ...
    printf("RMGR: loading task %s from 0x%08x-0x%08x\n      to [ ", name,
           mb_mod[i].mod_start, mb_mod[i].mod_end);
    exec_ret = exec_load(alloc_exec_read, alloc_exec_read_exec, &e, &exec_info);
    if (exec_ret)
      /* if it failed, try loading it as an PE file */
      exec_ret =
          exec_load_pe(alloc_exec_read, alloc_exec_read_exec, &e, &exec_info);
    ...
    /* copy mb_info */
    check((mbi = (struct grub_multiboot_info *)alloc_from_page(
               address, sizeof(struct grub_multiboot_info))));
    ...
    *mbi = mb_info;
    ...
    if (mb_info.flags & MB_INFO_VIDEO_INFO) {
      ...
      if (mb_info.vbe_mode_info) {
        if ((mbi_vbe_mode = (struct vbe_mode *)alloc_from_page(
                 address, sizeof(struct vbe_mode))) &&
            (mbi_vbe_cont = (struct vbe_controller *)alloc_from_page(
                 address, sizeof(struct vbe_controller)))) {
          ...
          *mbi_vbe_cont = mb_vbe_cont;
          ...
          printf("RMGR: ERROR: can't pass VBE video info "
                 "(needs %d bytes)\n",
                 sizeof(struct vbe_controller) + sizeof(struct vbe_mode));
          boot_error();
    ...
    if (bootquota[t.id.task].mods) {
      ...
      if (!(m = (struct multiboot_module *)alloc_from_page(
                address, j * sizeof(struct multiboot_module)))) {
      ...
      for (; j--; m++, mod_i++) {
        ...
        *m = mb_mod[mod_i];
        ...
        for (mod_mem = m->mod_start & L4_PAGEMASK; mod_mem < m->mod_end;
             mod_mem += L4_PAGESIZE) {
          if (!quota_check_mem(t.id.task, address, L4_PAGESIZE)) {
    ...
    sp -=
        1 + ((vm_offset_t)task_trampoline_end - (vm_offset_t)task_trampoline) /
                sizeof(*sp);
    memcpy(sp, task_trampoline,
           (vm_offset_t)task_trampoline_end - (vm_offset_t)task_trampoline);
    entry = (dword_t)sp;
    ...
    *--sp = 0; /* fake return address */
    ...
    printf("RMGR: starting task %s (#0x%x)\n"
           ...
           name, t.id.task, exec_info.entry, entry);

    t = l4_task_new(t, bootquota[t.id.task].mcp, (dword_t)sp, entry, myself);
...
static int alloc_exec_read_exec(void *handle, vm_offset_t file_ofs,
  ...
  for (address = mem_addr & L4_PAGEMASK; address <= mem_addr + mem_size;
       address += L4_PAGESIZE) {
  ...
  return mod_exec_read_exec(handle, file_ofs, file_size, mem_addr, mem_size,
                            section_type);
}
...
void mod_exec_init(struct multiboot_module *mod, unsigned mod_first,
                   unsigned mod_count) {
  unsigned i;
...
static void check_inside_myself(void *handle, vm_offset_t begin,
                                vm_offset_t end) {
  struct exec_task *e = (struct exec_task *)handle;
  ...
  if (!(begin >= (vm_offset_t)&_etext && begin <= (vm_offset_t)&_end &&
    ...
    boot_panic("\r\n"
               ...
               "            RMGR: %08x - %08x\n",
               begin, end, name, (vm_offset_t)&_start, (vm_offset_t)&_end);
  }
}

static inline int overlaps_range(vm_offset_t tst_begin, vm_offset_t tst_end,
...
static void check_overlaps_myself(void *handle, vm_offset_t begin,
                                  vm_offset_t end) {
  struct exec_task *e = (struct exec_task *)handle;
  int overlaps;

  if ((overlaps = overlaps_myself(begin, end)) || (begin >= mem_high) ||
      (end >= mem_high)) {
    int i;
    char *name, *errstr;
    char namebuf[45];
    ...
    printf("\nRMGR: %s: %08x-%08x: %s\n"
           "          RMGR: %08x-%08x\n",
           errstr, begin, end, name, (vm_offset_t)&_start, (vm_offset_t)&_end);

    for (i = first_task_module; i < mb_info.mods_count; i++) {
      name = strncpy(namebuf, mb_mod_name(i), sizeof(namebuf) - 1);
      printf("     module %02d: %08x-%08x: %s\n", i, mb_mod[i].mod_start,
             mb_mod[i].mod_end, name);
    }
    panic("RMGR: overlapping modules");
  }
}

int mod_exec_read(void *handle, vm_offset_t file_ofs, void *buf, vm_size_t size,
...
int mod_exec_read_exec(void *handle, vm_offset_t file_ofs, vm_size_t file_size,
