...
void rmgr(void) {
  ...
  pa = my_pager;
  pr = my_preempter;
  l4_thread_ex_regs(rmgr_super_id, (dword_t)mgr,
                    (dword_t)(mgr_stack + MGR_STACKSIZE), &pr, &pa, &dummy,
                    &dummy, &dummy);

  if (boot_mem_dump) {
    int c;
    l4_i386_ipc_call(rmgr_super_id, L4_IPC_SHORT_MSG,
                     RMGR_MEM_MSG(RMGR_MEM_INFO), 0, L4_IPC_SHORT_MSG, &dummy,
...
l4_threadid_t mb_mod_ids[MODS_MAX] = {
    L4_INVALID_ID,
};
    ...
    while (!err) {
      ...
      if (do_log && (debug_log_types & 1))
        printf("RMGR: Log: rcv from %x:%x: [0x%x, 0x%x], r = 0x%x\n",
               sender.lh.high, sender.lh.low, p.request, arg, result.msgdope);
      ...
      case RMGR_TASK:
        ...
        case RMGR_TASK_GET:
          ...
          n = l4_task_new(n,
                          p.proto.action == RMGR_TASK_GET ? sender.lh.low
                                                          : myself.lh.low,
                          0, 0, L4_NIL_ID);
...
#warning undefined L4 version
          ...
          if (p.proto.action == RMGR_TASK_SET_SMALL) {
            ...
            sched.sp.small =
                small_space_size | (p.proto.param * small_space_size * 2);
            s = L4_INVALID_ID;
...
#warning undefined L4 version
          ...
          mcp = p.proto.param > quota[partner.id.task].log_mcp
                    ? quota[partner.id.task].log_mcp
                    : p.proto.param;

          if (do_log)
            printf("RMGR: Log: creating task=%x:%x, esp=%x, "
                   "eip=%x, pager=%x:%x, mcp=%d\n",
                   n.lh.high, n.lh.low, *(dword_t *)str_buf,
                   *(dword_t *)(str_buf + 4),
                   ((l4_threadid_t *)(str_buf + 8))->lh.high,
                   ((l4_threadid_t *)(str_buf + 8))->lh.low, mcp);
          ...
          quota[task].log_mcp = p.proto.param > quota[partner.id.task].log_mcp
                                    ? quota[partner.id.task].log_mcp
                                    : p.proto.param;
...
#warning undefined L4 version
          ...
          n = l4_task_new(n, 0, *(dword_t *)str_buf, *(dword_t *)(str_buf + 4),
                          *(l4_threadid_t *)(str_buf + 8));
        ...
        case RMGR_TASK_SET_ID:
          ...
          if (result.msgdope == L4_IPC_DOPE(2, 1).msgdope) {
            ...
            for (b = first_not_bmod_task; a < first_not_bmod_free_modnr;
                 a++, b++) {
              if (strcmp(mb_mod_names[a], str_buf) == 0) {
                if (task_owner(p.proto.param) == partner.id.task) {
                  memcpy(&quota[p.proto.param], &quota[b], sizeof(quota[0]));
                  d1 = d2 = 0;
                  break;
                }
              }
      ...
      case RMGR_IRQ:
        ...
        case RMGR_IRQ_GET:
          if (p.proto.param < IRQ_MAX) {
            if (irq_alloc(p.proto.param, partner.id.task)) {
              ...
              err =
                  l4_i386_ipc_call(n, L4_IPC_SHORT_MSG, 1, 0, L4_IPC_SHORT_MSG,
                                   &e1, &e2, L4_IPC_NEVER, &result);
              check(err == 0);
        ...
        case RMGR_IRQ_FREE:
          if (p.proto.param < IRQ_MAX &&
            ...
            n = myself;
            n.id.lthread = LTHREAD_NO_IRQ(p.proto.param);
            err = l4_i386_ipc_call(n, L4_IPC_SHORT_MSG, 0, 0, L4_IPC_SHORT_MSG,
                                   &e1, &e2, L4_IPC_NEVER, &result);
            check(err == 0);
      ...
      case RMGR_MEM:
        ...
        case RMGR_MEM_FREE:
          ...
          if (arg >= 0x40000000 && arg < 0xC0000000) {
            ...
            if ((sender.id.task != memmap_owner_page(arg)) ||
              ...
              /* unmap fpage */
              l4_fpage_unmap(
                  l4_fpage(arg, L4_LOG2_PAGESIZE, L4_FPAGE_RW, L4_FPAGE_MAP),
        ...
        case RMGR_MEM_RESERVE:
          ...
          d1 = reserve_range(arg, partner.id.task, *(dword_t *)str_buf,
                             *(dword_t *)(str_buf + 4));
          break;
        case RMGR_MEM_INFO:
          show_mem_info();
        ...
        case RMGR_MEM_GET_PAGE0:
          d1 = 0;
          d2 = l4_fpage(0, L4_LOG2_PAGESIZE, L4_FPAGE_RW, L4_FPAGE_MAP).fpage;
    ...
    reply:
      ...
      if ((do_log && (debug_log_types & 4)) || (debug && d1 == (dword_t)-1))
        enter_kdebug("rmgr>");
      ...
      msg.data.rcv_size = MGR_BUFSIZE;
      msg.data.rcv_str = (dword_t)str_buf;
      err = l4_i386_ipc_reply_and_wait(sender, desc, d1, d2, &sender, &msg,
                                       &p.request, &arg,
