...
extern int rmgr_call_string(char *string, dword_t request, dword_t arg,
                            dword_t *out1, dword_t *out2);

extern inline int rmgr_call_string(char *string, dword_t request, dword_t arg,
                                   dword_t *out1, dword_t *out2) {
  l4_msgdope_t result;
  dword_t r1, r2;
  int err;
  ...
  err = l4_i386_ipc_call(rmgr_id, &msg, request, arg, L4_IPC_SHORT_MSG, &r1,
                         &r2, L4_IPC_NEVER, &result);
  if (err)
    return err;
...
extern inline int rmgr_call(dword_t request, dword_t arg, dword_t *out) {
  ...
  err = l4_i386_ipc_call(rmgr_id, L4_IPC_SHORT_MSG, request, arg,
                         L4_IPC_SHORT_MSG, &r1, &r2, L4_IPC_NEVER, &result);
...
int rmgr_init(void) {
  ...
  return (rmgr_call(RMGR_RMGR_MSG(RMGR_RMGR_PING, 0xbeef), 0,
                    &result.msgdope) == 0 &&
...
int rmgr_get_prio(l4_threadid_t dest, int *num) {
  ...
  l4_thread_schedule(dest, L4_INVALID_SCHED_PARAM, &foo_id, &foo_id, &p);
...
l4_taskid_t rmgr_task_new(l4_taskid_t dest, dword_t mcp_or_new_chief,
  ...
  static struct {
    ...
    dword_t d1, d2;
    l4_strdope_t data;
  } msg = {0, L4_IPC_DOPE(2, 1), L4_IPC_DOPE(2, 1), 0, 0, {16, 0, 0, 0}};
  ...
  if (deleting)
    err = l4_i386_ipc_call(rmgr_id, L4_IPC_SHORT_MSG,
                           RMGR_TASK_MSG(RMGR_TASK_DELETE, dest.id.task), 0,
                           L4_IPC_SHORT_MSG, &r1, &r2, L4_IPC_NEVER, &result);
  else
    err = l4_i386_ipc_call(
        rmgr_id, &msg, RMGR_TASK_MSG(RMGR_TASK_CREATE, mcp_or_new_chief),
        dest.lh.low, L4_IPC_SHORT_MSG, &r1, &r2, L4_IPC_NEVER, &result);
...
l4_taskid_t rmgr_task_new_with_prio(l4_taskid_t dest, dword_t mcp_or_new_chief,
  ...
  static struct {
    ...
    dword_t d1, d2;
    l4_strdope_t data;
  } msg = {0, L4_IPC_DOPE(2, 1), L4_IPC_DOPE(2, 1), 0, 0, {20, 0, 0, 0}};
  ...
  if (deleting)
    ...
    err = l4_i386_ipc_call(
        rmgr_id, &msg,
        RMGR_TASK_MSG(RMGR_TASK_CREATE_WITH_PRIO, mcp_or_new_chief),
        dest.lh.low, L4_IPC_SHORT_MSG, &r1, &r2, L4_IPC_NEVER, &result);
  }
...
int rmgr_set_task_id(char *module_name, l4_threadid_t thread_id) {
  ...
  err = rmgr_call_string(module_name,
                         RMGR_TASK_MSG(RMGR_TASK_SET_ID, thread_id.id.task),
                         thread_id.id.chief, &lo, &hi);
  return err;
}
