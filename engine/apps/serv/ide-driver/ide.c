...
#define is_error(stat)                                                         \
  (stat->error_bit || stat->ecc_occurred_bit || stat->write_fault_bit ||       \
...
int reg_to_irq(int irq) {
  ...
  ret = l4_ipc_receive(L4_INTERRUPT(irq), 0, &dummy, &dummy, &dummy,
                       L4_IPC_TIMEOUT_NULL, &dummydope);
...
void wait_for_interrupt(int irq) {
  dword_t dummy;
  l4_msgdope_t dummydope;
  int result;

  // printf("wait_for_irq %d", irq);
  result = l4_ipc_receive(L4_INTERRUPT(irq), 0, &dummy, &dummy, &dummy,
...
void switch_on_drive_lamp(void) { outb(SYS_PORT_A, inb(SYS_PORT_A) | 0x40); }

void switch_off_drive_lamp(void) { outb(SYS_PORT_A, inb(SYS_PORT_A) & ~0x40); }

/*
 * Command to hard disk
 */
void send_cmd_to_disk(int command, int drive_no, int sector_no, int sectors) {
  outb(SECTOR_NO_PORT, sector_no);
  outb(CYLINDER_LOW_PORT, sector_no >> 8);
  outb(CYLINDER_HIGH_PORT, sector_no >> 16);
  outb(DRIVE_HEAD_SELECT_PORT,
       ((sector_no >> 24) & 0xf) +
...
int open_controller(void) {
  if (reg_to_irq(IDE_HD_INTR))
    return -1;

  /* Do we have a controller? */
  outb(SECTOR_COUNT_PORT, 1);
  if (inb(SECTOR_COUNT_PORT) != 1)
    return -1;
  outb(SECTOR_COUNT_PORT, 2);
  if (inb(SECTOR_COUNT_PORT) != 2)
    return -1;
  outb(SECTOR_NO_PORT, 3);
  if (inb(SECTOR_NO_PORT) != 3)
    return -1;
  outb(SECTOR_NO_PORT, 4);
  if (inb(SECTOR_NO_PORT) != 4)
    return -1;
