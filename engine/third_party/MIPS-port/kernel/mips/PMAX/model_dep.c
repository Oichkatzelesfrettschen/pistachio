 ...
 * HISTORY
 * $Log:	model_dep.c,v $
 * Revision 2.21  93/11/17  17:52:39  dbg
 * 	Don't call mc_probe.  MC clock is found like other devices.
 * 	[93/06/14            dbg]
 * 
...
int d_find_systype(void)
{
	return string_to_int(prom_getenv("systype"));
}

int	(*find_systype)(void) = d_find_systype;
...
boolean_t isa_pmax(void) {return pmax_box;}

void kn01_model_dep(void)
{
	/*
...
void kn03_model_dep(void)
{
	...
	*((volatile unsigned *)(PHYS_TO_K1SEG(KN03_REG_IMSK))) = KN03_IM4;
	...
	asc_set_dmaops(0, kmin_dma_ops);
	asc_clock_speed_in_mhz[1] = 40;		/* really? */
