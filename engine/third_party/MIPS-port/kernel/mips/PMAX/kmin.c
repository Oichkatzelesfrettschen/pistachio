 ...
 * HISTORY
 * $Log:	kmin.c,v $
 * Revision 2.10  93/11/17  17:49:29  dbg
 * 	Converted for new clocks and timers.
 * 	[93/06/11            dbg]
 * 
...
void kmin_tc3intr(
	...
	struct mips_saved_state *ss)
{
	register unsigned int	intr = *(volatile unsigned int *)KMIN_INTR;
	...
	if (intr & KMIN_INTR_TIMEOUT)
		...
		mc_intr(modspl);
		splx(modspl);
		sys_clock_interrupt( (st & SR_KUo) != 0 );
	}
 ...
 *
 */
void kmin_slot_hand_fill(
	tc_option_t	*slot)
{
	register int	i;

	for (i = KMIN_SCSI_SLOT; i < KMIN_CLOCK_SLOT+1; i++) {
		slot[i].present = 1;
		slot[i].slot_size = 1;
		slot[i].rom_width = 1;
		slot[i].isa_ctlr = 0;
	...
	/* clock */
	bcopy("MC146818", slot[KMIN_CLOCK_SLOT].module_name, TC_ROM_LLEN+1);
	slot[KMIN_CLOCK_SLOT].k1seg_address = PHYS_TO_K1SEG(KMIN_SYS_CLOCK);
}
