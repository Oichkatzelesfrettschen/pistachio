 ...
 * HISTORY
 * $Log:	autoconf.c,v $
 * Revision 2.20  93/11/17  17:53:20  dbg
 * 	Configure clock device here for kn01.
 * 	[93/06/14            dbg]
 * 
...
void		identify_cpu(void);	/* forward */
cpu_subtype_t	identify_mipsbox(void);	/* forward */
void		fp_init(void);		/* forward */
...
void machine_init(void)
{
	...
	extern int hz;
	extern void (*identify_devices)(void);
...
#ifdef	MSERIES
void (*identify_devices)(void) = config_vme_routine;
#endif	/* MSERIES */


#ifdef	DECSTATION

void config_pmax_routine(void) 
{
	/*
	...
	/*
	 * Probe the clock chip - FIRST!  attach routines
	 * may use timeouts.
	 */
	configure_bus_device( "mc", 0xbd000000, 0, 4, "slot");
...
void config_3max_routine(void)
{
	extern void tc_autoconf(void);
...
void (*identify_devices)(void) = config_pmax_routine;
