 ...
 * HISTORY
 * $Log:	kn02.c,v $
 * Revision 2.10  93/11/17  17:50:04  dbg
 * 	Find time-of-day clock chip in a pseudo-TC slot, like the DZ.
 * 	[93/07/15            dbg]
 * 
...
void kn02_slot_hand_fill(
	tc_option_t	*slot)
{
	/*
	 *	DZ in slot 7
	 */
	slot[KN02_DZ_SLOT].present = 1;
	slot[KN02_DZ_SLOT].slot_size = 1;
	slot[KN02_DZ_SLOT].rom_width = 1;
	slot[KN02_DZ_SLOT].isa_ctlr = 0;
#if unsafe
	bcopy(0xbffc0410, slot[KN02_DZ_SLOT].module_name, TC_ROM_LLEN+1);
#endif
	bcopy("KN02    ", slot[KN02_DZ_SLOT].module_name, TC_ROM_LLEN+1);
	bcopy("DEC xxxx", slot[KN02_DZ_SLOT].module_id, TC_ROM_LLEN+1);
	slot[KN02_DZ_SLOT].k1seg_address = PHYS_TO_K1SEG(KN02_SYS_DZ);

	/*
	 *	Time-of-year clock in slot 8
	 */
	slot[KN02_CLOCK_SLOT].present = 1;
	slot[KN02_CLOCK_SLOT].slot_size = 1;
	slot[KN02_CLOCK_SLOT].rom_width = 1;
	slot[KN02_CLOCK_SLOT].isa_ctlr = 0;

	bcopy("MC146818", slot[KN02_CLOCK_SLOT].module_name, TC_ROM_LLEN+1);
	bcopy("DEC xxxx", slot[KN02_CLOCK_SLOT].module_id, TC_ROM_LLEN+1);
	slot[KN02_CLOCK_SLOT].k1seg_address = PHYS_TO_K1SEG(KN02_SYS_CLOCK);
