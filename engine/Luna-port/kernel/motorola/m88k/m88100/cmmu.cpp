...
#define CMMU_MARRIED 2   /* Know which cpu it belongs to. */
  ...
  unsigned batc[8];
#endif
} cmmu[MAX_CMMUS] = {{(void *)CMMU_I0, 0, 0, 0}, {(void *)CMMU_D0, 0, 1, 0},
                     {(void *)CMMU_I1, 1, 0, 0}, {(void *)CMMU_D1, 1, 1, 0},
                     {(void *)CMMU_I2, 2, 0, 0}, {(void *)CMMU_D2, 2, 1, 0},
                     {(void *)CMMU_I3, 3, 0, 0}, {(void *)CMMU_D3, 3, 1, 0}};
...
void show_sctr(unsigned value) {
  union {
    unsigned bits;
    struct {
      unsigned : 16, pe : 1, se : 1, pr : 1, : 13;
    } fields;
  } sctr;
  sctr.bits = value;
  raw_printf("%spe, %sse %spr]\n", sctr.fields.pe ? "" : "!",
             sctr.fields.se ? "" : "!", sctr.fields.pr ? "" : "!");
}
#endif
...
void cmmu_init(void) {
  ...
  for (cmmu_num = 0; cmmu_num < MAX_CMMUS; cmmu_num++) {
    if (!wprobe((vm_offset_t)cmmu[cmmu_num].cmmu_regs, -1)) {
       ...
       */
      tmp = !CMMU_SCTR_PE | /* not parity enable */
      ...
      cmmu[cmmu_num].cmmu_regs->sapr = cmmu[cmmu_num].cmmu_regs->uapr = tmp;

#if SHADOW_BATC
      cmmu[cmmu_num].batc[0] = cmmu[cmmu_num].batc[1] = cmmu[cmmu_num].batc[2] =
          cmmu[cmmu_num].batc[3] = cmmu[cmmu_num].batc[4] =
              cmmu[cmmu_num].batc[5] = cmmu[cmmu_num].batc[6] =
                  cmmu[cmmu_num].batc[7] = 0;
#endif
      cmmu[cmmu_num].cmmu_regs->bwp[0] = cmmu[cmmu_num].cmmu_regs->bwp[1] =
          cmmu[cmmu_num].cmmu_regs->bwp[2] = cmmu[cmmu_num].cmmu_regs->bwp[3] =
...
unsigned cmmu_get_idr(unsigned data) { return REGS(cpu_number(), data).idr; }
 ...
 * batc values.
 */
void cmmu_set_batc_entry(unsigned cpu, unsigned entry_no,
...
void cmmu_set_pair_batc_entry(
    unsigned cpu, unsigned entry_no,
    unsigned value) /* the value to stuff into the batc */
{
  CMMU_LOCK;
 ...
 */
void cmmu_flush_remote_tlb(unsigned cpu, unsigned kernel, vm_offset_t vaddr,
                           int size) {
  register s = splhigh();

  CMMU_LOCK;
  if ((unsigned)size > M88K_PGBYTES) {
    REGS(cpu, INST_CMMU).scr = REGS(cpu, DATA_CMMU).scr =
 ...
 */
void cmmu_pmap_activate(unsigned cpu, unsigned uapr,
...
union ssr {
  unsigned bits;
  struct {
    unsigned : 16, ce : 1, be : 1, : 4, wt : 1, sp : 1, g : 1, ci : 1, : 1,
        m : 1, u : 1, wp : 1, bh : 1, v : 1;
  } field;
};
...
union cssp {
  unsigned bits;
  struct {
    unsigned : 2, l : 6, d3 : 1, d2 : 1, d1 : 1, d0 : 1, vv3 : 2, vv2 : 2,
        vv1 : 2, vv0 : 2, : 12;
  } field;
};
...
union batcu {
  ...
  struct {                 /* block address translation register */
    unsigned int lba : 13, /* logical block address */
 ...
 */
void cmmu_show_translation(unsigned address, unsigned supervisor_flag,
  ...
  union {
    unsigned bits;
    struct {
      unsigned segment_table_index : 10, page_table_index : 10,
          page_offset : 12;
    } field;
  } virtual_address;
  unsigned value;
  ...
  if (thread != 0) {
    ...
    if (verbose_flag) {
      db_printf("[thread %x task %x map %x pmap %x UAPR is %x]\n", thread,
                thread->task, thread->task->map, thread->task->map->pmap,
                value);
    }
  } else {
    if (cmmu_num == -1) {
...
/******* LOOK AT THE BATC ** (if not a thread) **************/
  ...
  if (thread == 0) {
    ...
    if (ssr.field.v)
      db_printf("PROBE of 0x%08x returns phys=0x%x", address, cmmu_regs->sar);
    else
  ...
  /******* INTERPRET AREA DESCRIPTOR *********/
  {
    ...
    if (thread == 0)
      ...
      db_printf("THREAD %x", thread);
    db_printf(" %cAPR: SegTbl: 0x%x000p", supervisor_flag ? 'S' : 'U',
              apr_template.field.st_base);
    if (apr_template.field.wt)
      db_printf(", WTHRU");
    else
      db_printf(", !wthru");
  ...
  /****** ACCESS SEGMENT TABLE AND INTERPRET SEGMENT DESCRIPTOR  *******/
  {
    ...
    value |=
        virtual_address.field.segment_table_index * sizeof(struct sdt_entry);
    ...
    db_printf("SEG DESC @0x%x: PgTbl: 0x%x000", value,
              std_template.sdt_desc.table_addr);
    if (std_template.sdt_desc.wt)
      db_printf(", WTHRU");
    else
      db_printf(", !wthru");
    if (std_template.sdt_desc.sup)
      db_printf(", S-PROT");
    else
      db_printf(", UserOk");
    if (std_template.sdt_desc.g)
      db_printf(", GLOBAL");
    else
      db_printf(", !global");
    if (std_template.sdt_desc.no_cache)
      db_printf(", $INHIBIT");
    else
      db_printf(", $ok");
    if (std_template.sdt_desc.prot)
      db_printf(", W-PROT");
    else
      db_printf(", WriteOk");
    if (std_template.sdt_desc.dtype)
      db_printf(", VALID");
    else
      db_printf(", !valid");
    db_printf(".\n");
  ...
  /******* PAGE TABLE *********/
  {
    ...
    value |= virtual_address.field.page_table_index * sizeof(struct pt_entry);
    ...
    db_printf("PAGE DESC @0x%x: page @%x000", value, pte_template.pte.pfn);
    if (pte_template.pte.wired)
      db_printf(", WIRE");
    else
      db_printf(", !wire");
    if (pte_template.pte.wt)
      db_printf(", WTHRU");
    else
      db_printf(", !wthru");
    if (pte_template.pte.sup)
      db_printf(", S-PROT");
    else
      db_printf(", UserOk");
    if (pte_template.pte.g)
      db_printf(", GLOBAL");
    else
      db_printf(", !global");
    if (pte_template.pte.ci)
      db_printf(", $INHIBIT");
    else
      db_printf(", $ok");
    if (pte_template.pte.modified)
      db_printf(", MOD");
    else
      db_printf(", !mod");
    if (pte_template.pte.pg_used)
      db_printf(", USED");
    else
      db_printf(", !used");
    if (pte_template.pte.prot)
      db_printf(", W-PROT");
    else
      db_printf(", WriteOk");
    if (pte_template.pte.dtype)
      db_printf(", VALID");
    else
      db_printf(", !valid");
    db_printf(".\n");
...
void cmmu_cache_state(unsigned addr, unsigned supervisor_flag) {
  static char *vv_name[4] = {"exclu-unmod", "exclu-mod", "shared-unmod",
                             "invalid"};
  int cmmu_num;
  CMMU_LOCK;
  for (cmmu_num = 0; cmmu_num < MAX_CMMUS; cmmu_num++) {
    ...
    db_printf("cmmu #%d %s cmmu for cpu %d.\n", cmmu_num,
              cmmu[cmmu_num].which ? "data" : "inst", cmmu[cmmu_num].cmmu_cpu);
    R->sar = addr;
