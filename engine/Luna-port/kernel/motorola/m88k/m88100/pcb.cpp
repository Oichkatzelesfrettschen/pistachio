 ...
 * HISTORY
 * $Log:	pcb.c,v $
 * Revision 2.8  93/11/17  17:57:05  dbg
 * 	Delinted.
 * 	[93/10/15            dbg]
 *
...
#include <mach/boolean.h>             /* boolean.h */
...
void pcb_module_init(void) {
   ...
   */
  pcb_zone =
      zinit(sizeof(struct pcb) + 8, THREAD_MAX * (8 + sizeof(struct pcb)),
            THREAD_CHUNK * (8 + sizeof(struct pcb)), FALSE, "m88100 pcb state");
}
...
void pcb_init(thread_t thread) {
  ...
  *p = PCB_MAGIC;
   ...
   */
  pcb->kernel_state.pcb_sp =
      (unsigned int)thread->kernel_stack + KERNEL_STACK_SIZE - 8;
  *((struct m88100_saved_state **)(pcb->kernel_state.pcb_sp)) = saved_state;
 ...
 */
kern_return_t thread_setstatus(thread_t thread, int flavor,
                               thread_state_t tstate, natural_t count) {
  ...
  bcopy(&state->r[0], &saved_state->r[0], 32 * sizeof(int));

  /* ignore sxip */
  saved_state->snip = state->snip;
  saved_state->sfip = state->sfip;
  saved_state->epsr =
      (saved_state->epsr & ~PSR_SET_BY_USER) | (state->psr & PSR_SET_BY_USER);
  return KERN_SUCCESS;
}
 ...
 */
kern_return_t thread_getstatus(thread_t thread, int flavor,
                               ...
                               natural_t *count)      /* IN/OUT */
{
  ...
  if (flavor == 0 || flavor == M88100_THREAD_STATE) {
    ...
    bcopy(&saved_state->r[0], &state->r[0], 32 * sizeof(int));
 ...
 */
vm_offset_t set_user_regs(vm_offset_t stack_base, /* low address */
                          vm_offset_t stack_size, natural_t *entry,
                          vm_size_t arg_size) {
  ...
  bzero(saved_state, sizeof(struct m88100_saved_state));
 ...
 *
 */
void stack_attach(thread_t thread, vm_offset_t kernel_stack,
                  no_return (*continuation)(thread_t)) {
  ...
  *(struct m88100_saved_state **)sp = &(thread->pcb->user_state);

/* record twice for debugging */
#if MACH_KDB
  *((struct m88100_saved_state **)sp + 1) = &(thread->pcb->user_state);
#endif
 ...
 */
thread_t switch_context(thread_t old, continuation_t continuation,
                        thread_t new) {
  ...
  return Switch_context(old, continuation, new, &(old->pcb->kernel_state),
                        &(new->pcb->kernel_state));
}
