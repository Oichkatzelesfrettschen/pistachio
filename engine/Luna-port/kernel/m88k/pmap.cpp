 ...
 *
 */
void _pmap_activate(register pmap_t pmap, register thread_t th,
                    register int my_cpu) {
 ...
 *
 */
void _pmap_deactivate(register pmap_t pmap, register thread_t th,
                      register int my_cpu) {
  if (pmap != kernel_pmap) {
...
static int unsigned m88k_protection(pmap_t map, vm_prot_t prot) {
...
static pt_entry_t *pmap_expand_kmap(vm_offset_t virt, vm_prot_t prot) {
  int aprot;
  sdt_entry_t *sdt;
  ...
  ((sdt_entry_template_t *)(sdt + SDT_ENTRIES))->bits =
      (vm_offset_t)kpdt_ent | aprot | DT_VALID;
 ...
 *
 */
vm_offset_t pmap_map(register vm_offset_t virt, register vm_offset_t start,
                     register vm_offset_t end, register vm_prot_t prot
#ifdef OMRON_PMAP
                     ,
  ...
  if ((pmap_con_dbg & (CD_MAP | CD_FULL)) == (CD_MAP | CD_FULL))
    printf("(pmap_map :%x) phys address from %x to %x mapped at virtual %x, "
           "prot %x\n",
  ...
  for (num_phys_pages = npages; num_phys_pages > 0; num_phys_pages--) {
    ...
    if ((pte = pmap_pte(kernel_pmap, virt)) == PT_ENTRY_NULL)
      if ((pte = pmap_expand_kmap(virt, VM_PROT_READ | VM_PROT_WRITE)) ==
          PT_ENTRY_NULL)
 ...
 *
 */
vm_offset_t pmap_map_batc(register vm_offset_t virt, register vm_offset_t start,
                          register vm_offset_t end, register vm_prot_t prot,
                          register unsigned cmode) {
  int aprot;
  unsigned num_phys_pages;
  ...
  if ((pmap_con_dbg & (CD_MAPB | CD_FULL)) == (CD_MAPB | CD_FULL))
    printf("(pmap_map_batc :%x) phys address from %x to %x mapped at virtual "
           "%x, prot %x\n",
           current_thread(), start, end, virt, prot);
#endif
...
#ifdef DBG
    if ((pmap_con_dbg & (CD_MAPB | CD_FULL)) == (CD_MAPB | CD_FULL))
      printf("(pmap_map_batc :%x) num_phys_pg=%X, virt=%X, aligne V=%d, "
             "phys=%X, aligne P=%d\n",
             current_thread(), num_phys_pages, virt, BATC_BLK_ALIGNED(virt),
             phys, BATC_BLK_ALIGNED(phys));
#endif
 ...
 */
void pmap_bootstrap(vm_offset_t load_start,  /* IN */
                    ...
                    vm_offset_t *virt_end)   /* OUT */
{
  ...
  sdt_entry_t *kmap;
  vm_offset_t vaddr, kmapva, kpdt_phys, s_text, e_text, kernel_pmap_size;
  register int i;
  ...
  if (!PAGE_ALIGNED(load_start)) {
    printf("pmap_bootstrap : \"load_start\" not on the m88k page boundary : "
           "0x%x\n",
           load_start);
  ...
  *virt_start += kernel_pmap_size;
  ...
  e_text =
      load_start + ((unsigned)&etext -
                    (unsigned)&kernelstart); /* paddr of end of text section*/
...
#define PMAPER pmap_map_batc
  ...
  vaddr = PMAPER((vm_offset_t)&kernelstart, s_text, e_text,
                 VM_PROT_WRITE | VM_PROT_READ, CACHE_GLOBAL);

  vaddr = PMAPER(vaddr, e_text, (vm_offset_t)kmap, VM_PROT_WRITE | VM_PROT_READ,
                 CACHE_GLOBAL);
  ...
  for (; ptable->size != 0xffffffffU; ptable++)
    if (ptable->size)
      PMAPER(ptable->virt_start, ptable->phys_start,
             ptable->phys_start + ptable->size, ptable->prot,
             ptable->cacheability);
...
pmap_t pmap_create(vm_size_t size) {
   ...
   */
  pmap_cache_ctrl(kernel_pmap, (vm_offset_t)segdt,
                  (vm_offset_t)segdt + SDT_SIZE, CACHE_INH);
  /*
...
static void pmap_free_tables(pmap_t pmap) {
  ...
  /*
    ...
    because of integer overflow/wraparound if VM_MAX_USER_ADDRESS is near
    0xffffffff
  */
...
static void pmap_remove_range(pmap_t pmap, vm_offset_t s, vm_offset_t e) {
  ...
  for (va = s; va < e; va += PAGE_SIZE) {
    ...
    if (!SDT_VALID(sdt)) {
      ...
      if (va <= e - (1 << SDT_SHIFT))
        va += (1 << SDT_SHIFT) -
              PAGE_SIZE; /* no page table, skip to next seg entry */
      else               /* wrap around */
        break;
      continue;
    }
 ...
 *
 */
void pmap_enter(register pmap_t pmap, vm_offset_t va, vm_offset_t pa,
...
#endif
      template.bits =
          DT_VALID | ap | M88K_TRUNC_PAGE(pa) | CACHE_GLOBAL /*XXX*/;
    if (wired)
 ...
 * at VM fault time.
 */
void pmap_copy(pmap_t dst_pmap, pmap_t src_pmap, vm_offset_t dst_addr,
 ...
 */
pmap_t pmap_kernel(void) { return (kernel_pmap); } /* pmap_kernel() */
...
void pmap_copy_page(vm_offset_t src, vm_offset_t dst) {
  ...
  for (i = 0; i < ptes_per_vm_page;
       i++, src += M88K_PGBYTES, dst += M88K_PGBYTES) {
 ...
 *
 */
void copy_to_phys(register vm_offset_t srcva, register vm_offset_t dstpa,
 ...
 *
 */
void copy_from_phys(register vm_offset_t srcpa, register vm_offset_t dstva,
                    register int bytecount) {
 ...
 */
void pmap_pageable(pmap_t pmap, vm_offset_t start, vm_offset_t end,
                   boolean_t pageable) {
#ifdef lint
  pmap++;
  start++;
...
is_mod_Retry:
  ...
  if ((boolean_t)pmap_modify_list[pfi]) {
    ...
    if ((pmap_con_dbg & (CD_IMOD | CD_NORM)) == (CD_IMOD | CD_NORM))
      printf(
          "(pmap_is_modified :%x) already cached a modify flag for this page\n",
          current_thread());
#endif
    SPLX(spl);
    return (TRUE);
...
void pagemove(vm_offset_t from, vm_offset_t to, int size) {
  ...
  while (size > 0) {
    ...
    if ((dstpte = pmap_pte(kernel_pmap, (vm_offset_t)to)) == PT_ENTRY_NULL)
      if ((dstpte = pmap_expand_kmap(
               (vm_offset_t)to, VM_PROT_READ | VM_PROT_WRITE)) == PT_ENTRY_NULL)
    ...
    if ((pmap_con_dbg & (CD_PGMV | CD_FULL)) == (CD_PGMV | CD_FULL))
      printf("(pagemove :%x) srcpte @ 0x%x = %x dstpte @ 0x%x = %x\n",
             current_thread(), (unsigned)srcpte, *(unsigned *)srcpte,
             (unsigned)dstpte, *(unsigned *)dstpte);
                             ...
                             vm_machine_attribute_val_t *value) /* IN/OUT */
{
  ...
  case MATTR_VAL_OFF: /* (generic) turn attribute off */
    ...
    pmap_attribute_update(pmap, v, round_page(address + size), MATTR_CACHE,
                          MATTR_VAL_OFF);
  ...
  case MATTR_VAL_ICACHE_FLUSH: /* flush from instruction cache(s) */
    ...
    if (v) {
      v = PHYS_TO_K0(v);
      if (*value == MATTR_VAL_CACHE_FLUSH || *value == MATTR_VAL_ICACHE_FLUSH)
        mipscache_Iflush(v, size);
      if (*value == MATTR_VAL_CACHE_FLUSH || *value == MATTR_VAL_DCACHE_FLUSH)
...
static void check_map(pmap_t map, vm_offset_t s, vm_offset_t e, char *who) {
  vm_offset_t va, old_va, phys;
  pv_entry_t pv_h, pv_e, saved_pv_e;
  pt_entry_t *ptep;
  boolean_t found;
  int loopcnt;
  ...
  for (va = s; va < e; va += PAGE_SIZE) {
    ...
    if (map == kernel_pmap &&
        va < round_page((vm_offset_t)(pmap_modify_list +
                                      (pmap_phys_end - pmap_phys_start))))
      continue;
    ...
    if (pv_h->pmap != PMAP_NULL) {
      ...
      while (pv_e != PV_ENTRY_NULL) {
        ...
        if (pv_e->pmap == map && pv_e->va == va) {
          if (found) {
            printf("check_map: Duplicate PV list entries at 0x%X and 0x%X in "
                   "PV list 0x%X.\n",
    ...
    if (!found) {
      printf("check_map: Mapping for pmap 0x%x VA 0x%X Phys 0x%X does not "
             "appear in PV list 0x%X.\n",
             map, va, phys, pv_h);
    }
  }
...
#define PRINT_SDT(p)                                                           \
  printf("%08X : ", ((sdt_entry_template_t *)p)->bits);                        \
  printf("table adress=0x%X, prot=%d, dtype=%d\n", M88K_PTOB(p->table_addr),   \
         p->prot, p->dtype);

#define PRINT_PDT(p)                                                           \
  printf("%08X : ", ((pte_template_t *)p)->bits);                              \
  printf("frame num=0x%X, prot=%d, dtype=%d, wired=%d, modified=%d, "          \
         "pg_used=%d\n",                                                       \
         p->pfn, p->prot, p->dtype, p->wired, p->modified, p->pg_used);
 ...
 *
 *  History:
 *		'90.7.25	Fuzzy	Null sdt entry skip, and skip count
 *print.
 *
 *	Function:
...
void pmap_print(pmap_t pmap) {
  ...
  printf("	sdt_paddr: 0x%X; sdt_vaddr: 0x%X; ref_count: %d; lock: 0x%X\n",
         (unsigned)sdtp, (unsigned)sdtv, pmap->ref_count,
         *((long *)(&pmap->lock)));

#ifdef statistics_not_yet_maintained
  printf("	statistics: pagesize %d: free_count %d; "
         "active_count %d; inactive_count %d; wire_count %d\n",
         pmap->stats.pagesize, pmap->stats.free_count, pmap->stats.active_count,
         pmap->stats.inactive_count, pmap->stats.wire_count);

  printf("	zero_fill_count %d; reactiveations %d; "
         "pageins %d; pageouts %d; faults %d\n",
         pmap->stats.zero_fill_count, pmap->stats.reactivations,
         pmap->stats.pageins, pmap->stats.pageouts, pmap->stats.fault);

  printf("	cow_faults %d, lookups %d, hits %d\n", pmap->stats.cow_faults,
         pmap->stats.loopups, pmap->stats.faults);
#endif
...
void pmap_print_trace(pmap_t pmap, vm_offset_t va, boolean_t long_format) {
  ...
  if (long_format) {
    ...
    for (i = 0; i < PDT_ENTRIES; i++, ptep++) {
      if (prev_entry == ((pte_template_t *)ptep)->bits && PDTIDX(va) != i &&
          i != PDT_ENTRIES - 1) {
        n_dup_entries++;
...
unsigned int pmap_free_pages(void) { return atop(avail_end - avail_next); }
 ...
 *      Set BATC
 */
void pmap_set_batc(pmap_t pmap, boolean_t data, int i, vm_offset_t va,
                   vm_offset_t pa, boolean_t super, boolean_t wt,
                   boolean_t global, boolean_t ci, boolean_t wp,
                   boolean_t valid) {
...
void use_batc(task_t task, boolean_t data, /* for data-cmmu ? */
              int i,                       /* batc number */
 ...
 *	The pmap must be locked.
 */
void pmap_range_remove(pmap_range_t *ranges, vm_offset_t start,
                       vm_offset_t end) {
  register pmap_range_t range, *prev;
