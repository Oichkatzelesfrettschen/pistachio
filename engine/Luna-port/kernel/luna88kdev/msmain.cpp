 ...
 * HISTORY
 * $Log:	msmain.c,v $
 * Revision 2.10  93/11/17  17:37:20  dbg
...
#define mouse_buffer_unlock() simple_unlock(&mouse_data_buffer.lock)
  ...
  struct mouse_data *next_free, /* Elements inserted here.. */
...
static void insert_into_data_buffer(unsigned A, int B, int C, char *where) {
  ...
  mouse_data_buffer.next_free->time = sys_clock->time.seconds;
...
int mouse_receive(unsigned data) {
  ...
  if (in)
    raw_printf("[K IN IN IN IN IN IN IN\n");
  in = 1;
#endif
  ...
  case 2: /* this is the last of the three bytes */
    ...
    if (!mouse_scan)
      insert_into_data_buffer(button_data_byte, x_data_byte, y_data_byte,
                              "regular");
    else {
 ...
 */
static int return_raw_data(unsigned nodelay, unsigned latest, unsigned count,
...
#ifdef MOUSE_DEBUG
      if (mouse_debug)
        raw_printf("[K sleeping waiting for data]\n");
#endif
...
#ifdef MOUSE_DEBUG
  if (mouse_debug)
    raw_printf("[K return_raw_data returns %d]\n", retval);
#endif
  return retval;
}

int mouse_set_status(int dev, int flavor, dev_status_t status,
                     unsigned status_count) {
#ifdef MOUSE_DEBUG
  if (mouse_debug)
    raw_printf("[K mouse_set_status(flavor=%x)]\n", flavor);
#endif
...
int mouse_get_status(int dev, int flavor, dev_status_t status, /* OUT    */
                     unsigned int *status_count)               /* IN/OUT */
{
  int elements;
#ifdef MOUSE_DEBUG
  if (mouse_debug)
    raw_printf("[K mouse_get_status(flavor=%x)]", flavor);
#endif
...
#ifdef MOUSE_DEBUG
    if (mouse_debug)
      raw_printf("[K got w/b %d elements]\n", elements);
#endif
    return D_SUCCESS;
...
#ifdef MOUSE_DEBUG
    if (mouse_debug)
      raw_printf("[K got wo/b %d elements]\n", elements);
#endif
    return D_SUCCESS;
...
#ifdef MOUSE_DEBUG
    if (mouse_debug)
      raw_printf("[K got last w/b %d elements]\n", elements);
#endif
    return D_SUCCESS;
...
#ifdef MOUSE_DEBUG
    if (mouse_debug)
      raw_printf("[K got last wo/b %d elements, status_count is %d]\n",
