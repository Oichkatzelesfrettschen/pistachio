...
int cnwrite(unsigned dev, struct uio *uio) { return utwrite(dev, uio); }
...
io_return_t cngetstat(unsigned dev, int flavor,
...
io_return_t cnsetstat(unsigned dev, int flavor,
                      int *data, /* pointer to OUT array */
                      unsigned int count) {
...
static void cnxint_cons(int c) { cnxint(&cons_tty[CONSOLE], c); }

static void cnxint_term(int c) { cnxint(&cons_tty[TERM], c); }

static void cnxint_kbd(int c) { cnxint(&cons_tty[KBD], c); }
...
/*ARGSUSED*/
int cnmmap(unsigned dev, off_t off, int prot) { return -1; }
