...
void luna_show_keyboard_state(void) {
  ...
  if (kb_state.caps)
    db_printf(" CAPS");
  if (kb_state.leftshift)
    db_printf(" LEFT");
  if (kb_state.rightshift)
    db_printf(" RIGHT");
...
static unsigned do_fullencoded(unsigned keycode, unsigned char *buf,
                               int shift) {
  unsigned repkeycode;
  inline unsigned get_translation(unsigned keycode, unsigned char *buf) {
    ...
    for (i = 0; i < EXPANSIONS; i++) {
      if (encoded_expansions[i].keycode == keycode &&
        ...
        for (count = 0; count < MAX_EXPANSION_LEN && *ptr;
             count++, buf++, ptr++)
          *buf = *ptr;
        return count;
      }
...
unsigned kbencode(unsigned keycode, unsigned char *buf) {
  ...
  case KEYCODE_CTRL:
    kb_state.ctrl = !release(keycode);
    break;
  case KEYCODE_KANA:
    kb_state.kana = !release(keycode);
    break;
  case KEYCODE_RIGHT_SHIFT:
    kb_state.rightshift = !release(keycode);
    break;
  case KEYCODE_LEFT_SHIFT:
    kb_state.leftshift = !release(keycode);
    break;
  case KEYCODE_CAPS:
    kb_state.caps = !release(keycode);
    break;
  case KEYCODE_ALT:
    kb_state.alt = !release(keycode);
    break;
  default:
  ...
  if (kb_state.alt && kb_state.leftshift && kb_state.ctrl)
    switch (keycode) {
...
void keyboard_setencoded(void) { kb_state.encoding_mode = ENCODED; }
