 ...
 * HISTORY
 * $Log:	fb.c,v $
 * Revision 2.5  93/11/17  17:37:03  dbg
...
io_return_t fb_open(int dev, int flag) {
  if (fb_framemem == 0)
    return D_NO_SUCH_DEVICE;
  return D_SUCCESS;
}

io_return_t fb_close(int dev) {
 ...
 * Return -1 if illegal.
 */
vm_offset_t fb_mmap(int dev, vm_offset_t pos, vm_prot_t prot) {
  ...
  return pmap_phys_address(pg_pfn);
}

io_return_t fb_set_status(int dev, dev_flavor_t flavor, dev_status_t ptr,
                          natural_t size) {
  switch (flavor) {
  default:
    return D_NO_SUCH_DEVICE;
...
io_return_t fb_get_status(int dev, dev_flavor_t flavor, dev_status_t ptr,
                          natural_t *size) {
  switch (flavor) {
  default:
    return D_NO_SUCH_DEVICE;
...
#define BM24_NAME_SHIFT 24
    ...
    if (!bad) {
      fb_planes = 24;
      fb_framemem =
          m88k_btop((unsigned)((unsigned)BMAP_END24P - (unsigned)BMAP_START));
  ...
  if (addr != BMAP_START) {
    static unsigned fb_chkaddr[] = {BMAP_BMAP0, BMAP_BMAP1, BMAP_BMAP2,
                                    BMAP_BMAP3, BMAP_BMAP4, BMAP_BMAP5,
                                    BMAP_BMAP6, BMAP_BMAP7};
...
struct bus_driver fb_driver = {fbprobe,  /* see if the driver is there */
