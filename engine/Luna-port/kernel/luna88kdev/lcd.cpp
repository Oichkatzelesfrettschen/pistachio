 ...
 */
static __inline__ void write_lcd_data(data) unsigned char data;
{
  m88k_psr_type psr;
 ...
 * Only used in one place, so inlined.
 */
static __inline__ void write_lcd_cmd(cmd) unsigned cmd;
{
  m88k_psr_type psr;
 ...
 */
static void output_lcd_data(data, len) unsigned char *data;
int len;
{
  while (len-- > 0) {
    unsigned char c;
...
void lcd_slave() {
  static unsigned char lcd_init_string[] = {
      ...
      LCD_CTRL, LCD_ENTRY_CMD, LCD_CTRL, LCD_ON_CMD, /* turn it on */
