...
static void timeout_routine(volatile int *timeoutflag) { *timeoutflag = 1; }

/*
 * Called from "luna88k/machdep.c"
 */
void determine_cpu_speed() {
...
#define plural(num) ((num) == 1 ? "" : "s")
  printf("%s MC88100 CPU%s RUNNING AT %sMHz.\n", num[initialized_cpus],
         plural(initialized_cpus), speed);
}

static void batterychk() {
...
time_spec_t luna_clock_read(void) {
  ...
  *(volatile char *)OBIO_CAL_CTL &= ~RTC_RD;
  ...
  time.seconds = ymd_to_seconds(year, month, day, hour, mins, secs);
  time.nanoseconds = 0;
 ...
 */
void luna_clock_write(mach_clock_t clock, time_spec_t new_time) {
  ...
  seconds_to_ymd(new_time.seconds, &year, &month, &day, &hour, &mins, &secs,
                 &dow);
...
struct clock_ops luna_clock_ops = {luna_clock_setresolution, luna_clock_write,
                                   luna_clock_enable_interrupts};

void luna_clock_init() {
...
void luna_clock_setresolution(mach_clock_t clock) {
  /* Cannot change resolution */
}

void luna_clock_enable_interrupts(mach_clock_t clock) { start_clock(); }

io_return_t clk_open(int dev) {
  if (dev != 0)
...
io_return_t clk_close(int dev) { return D_SUCCESS; }

io_return_t clk_getstat(int dev, int flavor, dev_status_t stat,
                        natural_t *count) {
  return clock_getstat(&luna_clock, flavor, stat, count);
}

io_return_t clk_setstat(int dev, int flavor, dev_status_t stat,
                        natural_t count) {
  return clock_setstat(&luna_clock, flavor, stat, count);
}

vm_offset_t clk_mmap(int dev, vm_offset_t off, vm_prot_t prot) {
  return clock_map_page(&luna_clock, off, prot);
}

io_return_t clk_info(int dev, int flavor, mach_clock_t *info) {
