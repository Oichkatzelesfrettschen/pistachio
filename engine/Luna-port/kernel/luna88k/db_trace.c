...
char *m88k_exception_name( unsigned vector)
{
	...
	case  64: return "Ddb break";
	case  65: return "Ddb trace";
	case  66: return "Ddb trap";
	case 114: return "FPU precise";
	case 115: return "FPU imprecise";
	case 128: return "Syscall";
	case 129: return "Mach syscall";
    }
}
    ...
    char *modif)
{
    ...
    while (modif && *modif) switch (*modif++)
    {
	  case 'd':
	    #ifdef TRACE_DEBUG
	  ...
	  case 't': style = Thread ; break;
	  case 'f': style = Frame  ; break;
	  case 'c': style = CPU    ; break;
	...
	{
	    db_printf("thread 0x%x blocked with continuation 0x%x; ",
		arg.thread, arg.thread->swap_func);
	    if (trace_flags & TRACE_USER_FLAG)
	    {
