...
static char *bytes[] = {
    "____", "___x", "__x_", "__xx", "_x__", "_x_x", "_xx_", "_xxx",
  ...
  {                                                                            \
    if ((data_access_emulation_debug != 0) &&                                  \
        (data_access_emulation_debug == 0xffffffff ||                          \
         ...
         data_access_emulation_debug == (unsigned)current_task())) {           \
      stuff;                                                                   \
    }                                                                          \
  }
#else
...
void data_access_emulation(unsigned *eframe) {
  ...
  for (x = 0; x < 3; x++) {
    ...
    DAE_DEBUG(
        if (dmtx->dmt_write) DEBUG_MSG(
            "[cpu%d th=%x DMT%d=%x: st.%c %x to %x as [%s] %s %s]\n",
            cpu_number(), current_thread(), x, eframe[EF_DMT0 + x * 3],
            dmtx->dmt_das ? 's' : 'u', dmdx, dmax, bytes[dmtx->dmt_en],
            dmtx->dmt_doub1 ? "double" : "not double",
            dmtx->dmt_lockbar ? "xmem" : "not xmem");
        else DEBUG_MSG("[cpu%d th=%x DMT%d=%x: ld.%c r%d<-%x as [%s] %s %s]\n",
                       cpu_number(), current_thread(), x,
                       eframe[EF_DMT0 + x * 3], dmtx->dmt_das ? 's' : 'u',
    ...
    if (!dmtx->dmt_lockbar) {
      ...
      } else /* not pipeline #2 with a double */
      {
        ...
        else /* else it's a read */
        {
          ...
          if (reg == 0) {
            DAE_DEBUG(
                DEBUG_MSG("[cpu %d: no write to r0 done]\n", cpu_number()));
          } else {
 ...
 */
void vector_init(m88k_exception_vector_area *vector,
    ...
    if (skipcount-- <= 0) {
      ...
      if (db_lookup_task(raw_xpr_data[index].task) < 0) {
        ...
        db_printf("<task %x, fmt %x, arg %x, arg %x>\n",
                  raw_xpr_data[index].task, raw_xpr_data[index].fmt,
                  raw_xpr_data[index].arg1, raw_xpr_data[index].arg2);
      } else {
        char buffer[120];
        buffer[0] = '\0';
        db_read_bytes(raw_xpr_data[index].fmt, sizeof(buffer), buffer,
                      raw_xpr_data[index].task);
        buffer[sizeof(buffer) - 2] = '\n';
        buffer[sizeof(buffer) - 1] = '\0';

        db_printf(buffer, raw_xpr_data[index].arg1, raw_xpr_data[index].arg2);
      }
    }
    if (++index >= MAX_XPR_COUNT)
      index = 0;
