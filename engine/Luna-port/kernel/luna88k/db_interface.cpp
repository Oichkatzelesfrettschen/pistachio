 ...
 * Revision 2.13  93/11/17  17:35:27  dbg
 * 	Added include of <kern/machine.h>.
 * 	[93/10/15            dbg]
 *
 ...
 * 	ANSIfied, reorganized. Cleaned up includes.
 * 	[93/01/22            jfriedl]
 *
...
#include <kern/machine.h>   /* machine_slot		*/
...
static int luna_db_print_lock_acquire(void) {
  ...
  while (!(got = db_simple_lock_try(&ddb_print_lock)) &&
         count < 5 * 1024 * 1024) {
...
static int luna_dmx_print(unsigned t, unsigned d, unsigned a, unsigned no) {
  ...
  static char *mode[16] = {"?",  ".b", ".b", ".h", ".b", "?", "?", "?",
                           ".b", ".h", "?",  "?",  "?",  "?", "?", ""};
  static unsigned mask[16] = {0, 0xff, 0xff00,      0xffff,      0xff0000, 0,
                              0, 0,    0xff000000U, 0xffff0000U, 0,        0,
                              0, 0,    0,           0xffffffffU};
  ...
  if (XMEM(t)) {
    db_printf("xmem%s%s r%d(0x%x) <-> mem(0x%x),", XMEM_MODE(t), DAS(t), reg,
    ...
    if (MODE(t) == 0xf) {
      ...
      if (STORE(t))
        if (DOUB(t) && no == 2)
          db_printf("st.d%s -> mem(0x%x) (** restart sxip **)", DAS(t), a);
        else
          db_printf("st%s (0x%x) -> mem(0x%x)", DAS(t), d, a);
      else /* load */
        if (DOUB(t) && no == 2)
          db_printf("ld.d%s r%d <- mem(0x%x), r%d <- mem(0x%x)", DAS(t), reg, a,
                    reg + 1, a + 4);
        else
      ...
      if (STORE(t))
        ...
        db_printf("ld%s%s%s r%d <- mem(0x%x)", mode[MODE(t)],
                  SIGN(t) ? "" : "u", DAS(t), reg, a);
    }
  }
  return 0;
}

static void luna_db_print_frame(db_expr_t addr, boolean_t have_addr, int count,
                                char *modif) {
  ...
  if (s->vector == /*data*/ 3 || s->dmt0 & 1) {
    ...
    if ((s->dmt1 & 1) && (!surpress1)) {
      ...
      if ((s->dmt2 & 1) && (!surpress2)) {
        db_printf("dmt,d,a2: 0x%08x  0x%08x  0x%08x ", s->dmt2, s->dmd2,
                  s->dma2);
 ...
 */
static void luna_db_registers(db_expr_t addr, boolean_t have_addr, int count,
                              char *modif) {
  int cpu = ddb_master;
  int all = 0;
  unsigned char c;
 ...
 */
static unsigned char ddb_saved_palette[2][3] = {{/*WHITE*/ 0xff, 0xff, 0xff},
...
static void luna_db_release_console(void) {
  if (db_console_grabbed && !db_was_console_on) {
    ...
    bm_set_palette_colors(/*put previous values in */ 0,
                          ddb_saved_palette[0][0], ddb_saved_palette[0][1],
                          ddb_saved_palette[0][2], ddb_saved_palette[1][0],
                          ddb_saved_palette[1][1], ddb_saved_palette[1][2]);
    bm_align_screen();
  }
}
...
static void luna_db_clear_screen(void) {
...
static void luna_db_hold_down(void) {
  ...
  if (!(db_run_mode == STEP_NONE || db_run_mode == STEP_CONTINUE)) {
    ...
    NOISY(
        luna_db_str1(
            "(Because in step mode is nonzero, freezing all cpus except %d)\n",
            cpu);)
  } else {
    /* store idempotently */
    for (i = 0; i < NCPUS; i++)
...
static void luna_db_trap(int type, int code, int soft) {
  ...
  if (db_are_interrupts_disabled())
    luna_db_str1(
        "WARNING: (cpu %d) entered debugger with interrupts disabled\n", cpu);
  ...
  if (ddb_master == -1 && !soft) {
    ddb_master = cpu;
    ddb_depth = 1;
  } else if (ddb_master == cpu)

    ddb_depth++;
  db_simple_unlock(&ddb_entry_lock);

  NOISY2(
      luna_db_str1(cpu == ddb_master ? "(cpu %d) master\n" : "(cpu %d) slave\n",
                   cpu);)
 ...
 */
static boolean_t db_task_va_to_phys(vm_offset_t va, task_t task,
                                    vm_offset_t *pa) {
  pmap_t pm;
  vm_offset_t p;
...
int db_getc(void) { return mayget(); }
  ...
  for (i = 0; i < NCPUS; i++)
    if (machine_slot[i].is_cpu && (s = db_cur_exc_frame[i])) {
      ...
      db_find_xtrn_task_sym_and_offset(
          (db_addr_t)l, &name, (db_addr_t *)&offset,
...
/* probe my cache */
static void luna_db_peek(db_expr_t addr, boolean_t have_addr, int count,
                         char *modif) {
  int pa12;
  int cpu = cpu_number();
  int valmask;
...
static void luna_db_noise(db_expr_t addr, boolean_t have_addr) {
  if (!have_addr) {
    ...
    db_printf(
        "invalid noise level to luna_db_noisy; should be 0, 1, 2, or 3\n");
  else {
    db_noisy = addr;
...
boolean_t db_check_user_addr(vm_offset_t va) {
  vm_offset_t pa;
 ...
 * address
 */
boolean_t db_phy_eq(task_t t1, vm_offset_t va1, task_t t2, vm_offset_t va2) {
  vm_offset_t pa1, pa2;
  if (db_task_va_to_phys(va1, t1, &pa1) && db_task_va_to_phys(va2, t2, &pa2) &&
      (pa1 == pa2))
    return TRUE;
  return FALSE;
}

boolean_t db_check_access(task_t task, vm_offset_t va, unsigned int size) {
  vm_offset_t pa;
 ...
 */
static void luna_db_translate(db_expr_t addr, boolean_t have_addr,
                              unsigned count, char *modif) {
  char c;
  int verbose_flag = 0;
...
static struct db_command luna_cache_cmds[] = {{"iflush", luna_db_iflush, 0, 0},
...
void kdb_init(void) {
  ...
  /*
   * if dipswitch 4 (#3) is set, or if the "gimmeabreak" boot-time option has
   * been given, trap into the debugger via gimmeabreak
   */
...
char *db_task_name(task_t task) {
  ...
  while (ptr > limit && (i = db_trace_get_val((vm_offset_t)ptr, &word, task)) &&
         (word == 0)) {
  ...
  while (ptr > limit && (i = db_trace_get_val(ptr, &word, task)) &&
         (word != 0)) {
