 ...
 * HISTORY
 * $Log:	luna_init.c,v $
 * Revision 2.11  93/11/17  17:36:21  dbg
...
#define roundup(value, stride)                                                 \
  (((unsigned)(value) + (stride) - 1) & ~((stride) - 1))
...
void luna_init(void) {
  ...
  pmap_bootstrap((vm_offset_t)&kernelstart /* loadpt */, &avail_start,
                 &avail_end, &virtual_avail, &virtual_end);
}
