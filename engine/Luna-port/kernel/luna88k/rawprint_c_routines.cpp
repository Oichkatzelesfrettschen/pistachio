...
#define insert2buffer(character)                                               \
  (max_len == 0                                                                \
       ? (void)0                                                               \
       : ((max_len--, buffer) ? (void)(*out++ = (character)) : (void)out++))
 ...
 */
#ifdef __STDC__
static int show_num(unsigned char *buffer, unsigned value, int min_len,
                    int max_len, int base, int zerofill, int sign)
#else
...
#ifdef __STDC__
int VSNPRINTF_FCN_NAME(unsigned char *buffer, unsigned int max_len,
                       unsigned char const *format, va_list ap)
#else
  ...
  while (max_len != 0 && (C = *format++)) {
    ...
    case 'C': /* CAT-V like character */
    {
      ...
      if (isprint(value))
        ...
        insert2buffer('\\');
        chars =
            show_num(buffer ? out : 0, value, 3, max_arg_len, 8, 1, UNSIGNED);
        out += chars;
        max_len -= chars;
      }
      break;
    ...
    case 'x': /* hex */
    {
      int chars = show_num(buffer ? out : 0, NEXT_ARG(unsigned int), len,
                           max_arg_len, 16, zerofill, UNSIGNED);
      out += chars;
      max_len -= chars;
      break;
    }
    ...
    {
      int chars = show_num(buffer ? out : 0, NEXT_ARG(int), len, max_arg_len,
                           10, zerofill, SIGNED);
      out += chars;
      max_len -= chars;
      break;
    ...
    {
      int chars = show_num(buffer ? out : 0, NEXT_ARG(unsigned int), len,
                           max_arg_len, 10, zerofill, UNSIGNED);
      out += chars;
      max_len -= chars;
      break;
    }
    ...
    {
      int chars = show_num(buffer ? out : 0, NEXT_ARG(unsigned int), len,
                           max_arg_len, 8, zerofill, UNSIGNED);
      out += chars;
      max_len -= chars;
      break;
    }
    ...
    {
      int chars = show_num(buffer ? out : 0, NEXT_ARG(unsigned int), len,
                           max_arg_len, 2, zerofill, UNSIGNED);
      out += chars;
      max_len -= chars;
      break;
    }
    ...
    case 'g': {
      ...
      chars = show_num(buffer ? out : 0, (int)value, len, max_arg_len, 10, 0,
                       SIGNED);
      out += chars;
      max_len -= chars;
      max_arg_len -= chars;
      ...
      value -= (int)value;
      insert2buffer('.');
      chars = show_num(buffer ? out : 0, (int)(value * 1000.0), 3, max_arg_len,
                       10, 1, UNSIGNED);
      out += chars;
      max_len -= chars;
      break;
...
void raw_printf(fmt, va_alist) unsigned char *fmt;
va_dcl {
  va_list ap;
  unsigned char buffer[100];
