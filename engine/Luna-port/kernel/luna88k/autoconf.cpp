 ...
 * HISTORY
 * $Log:	autoconf.c,v $
 * Revision 2.12  93/11/17  17:34:46  dbg
 * 	Call luna_clock_init instead of startrtclock (new clocks).
 * 	[93/10/20            dbg]
 *
...
extern "C" void se_intr();
extern "C" void spc_intr();
extern "C" void fbintr();

struct bus_ctlr bus_master_init[] = {
    /* driver, name, unit, intr, addr+am, phys, adaptor, alive, flags */
    {&spc_driver, "spc", 0, reinterpret_cast<int (*)()>(spc_intr), 0x0, 0, 0,
     '?', 0, 0},
    {0}};
...
static inline unsigned char get_FUSErom_byte(unsigned index) {
  volatile struct {
    unsigned h_data : 4, h_dummy : 28;
    unsigned l_data : 4, l_dummy : 28;
...
char *bootarg(char *name) {
  ...
  if (boottime_arg_area == 0) {
    ...
    for (i = 0; i < boottime_arg_length; i++)
      if (boottime_arg_area[i] == ' ' || boottime_arg_area[i] == '\t')
...
static char *mmutypes[8] = {"Unknown (0)",  "Unknown (1)", "Unknown (2)",
                            "Unknown (3)",  "Unknown (4)", "M88200 (16K)",
                            "M88204 (64K)", "Unknown (7)"};
