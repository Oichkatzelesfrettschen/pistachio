 ...
 * HISTORY
 * $Log:	db_trace.c,v $
 * Revision 2.16  93/11/17  17:36:08  dbg
 * 	Delinted.
 * 	[93/10/15            dbg]
 *
 * Revision 2.15  93/03/09  17:56:44  danner
 ...
 * 	[93/02/22            jfriedl]
 *
 * Revision 2.14  93/02/05  07:50:29  danner
 * 	Fixes for tracing random user tasks.
 * 	[93/02/04            jfriedl]
 *
 * Revision 2.13  93/02/01  09:49:57  danner
 * 	Put 'signed' attribute to bitfield in is_jump_source_ok.
 * 	[93/01/26            jfriedl]
 *
 * Revision 2.12  93/01/26  18:02:53  danner
 ...
 * 	[93/01/20            danner]
 *
 * Revision 2.11  93/01/14  17:38:18  danner
...
#define SHOW_INSTRUCTION(Addr, Inst, Note)                                     \
  if (DEBUGGING_ON) {                                                          \
    ...
    luna_print_instruction((unsigned)(Addr), (Inst));                          \
    db_printf("\n");                                                           \
  }
#endif

/*
 * luna trace/register state interface for ddb.
 */

/* lifted from mips */
static void db_setf_regs(struct db_variable *vp, db_expr_t *valuep,
                         int op, /* read/write */
...
#define N(s, x) {s, (db_expr_t *)&(((db_regs_t *)0)->x), db_setf_regs}
 ...
 */
static unsigned m88k_instruction_info(unsigned instruction) {
  static struct {
...
static int hex_value_needs_0x(unsigned value) {
  int i;
  unsigned last = 0;
  unsigned char c;
...
int frame_is_sane(db_regs_t *regs) {
  ...
  /* stack sanity ... r31 must be nonzer, but must be word aligned */
...
/*
 * Read a word from address space tk at address addr.
 * Return 1 if was able to read, 0 otherwise.
 */
unsigned db_trace_get_val(vm_offset_t addr, unsigned *ptr, task_t tk) {
  jmp_buf_t db_jmpbuf;
  ...
  if (_setjmp(db_recover = &db_jmpbuf) != 0) {
    ...
    db_read_bytes((char *)addr, 4, (char *)ptr, tk);
    db_recover = prev;
    quiet_db_read_bytes = old_quiet_db_read_bytes;
    return 1;
  }
...
static void save_reg(int reg, unsigned value) {
#ifdef TRACE_DEBUG
  if (DEBUGGING_ON)
...
#define clear_local_saved_regs()                                               \
  {                                                                            \
    local_saved_list = trashed_list = 0;                                       \
  }
#define clear_global_saved_regs()                                              \
  {                                                                            \
    local_saved_list = global_saved_list = 0;                                  \
  }
 ...
 * wrong.
 */
static int is_jump_source_ok(unsigned return_to, unsigned jump_to, task_t tk) {
  unsigned flags;
   ...
   */
  if (!db_trace_get_val(return_to - 8, &instruction.rawbits, tk))
   ...
   */
  if (!db_trace_get_val(return_to - 4, &instruction.rawbits, tk))
    return JUMP_SOURCE_IS_BAD;
  flags = m88k_instruction_info(instruction.rawbits);
 ...
 *
 */
static int stack_decode(unsigned addr, unsigned *stack, task_t tk) {
  db_sym_t proc;
  ...
  extern char *who_is_task(task_t t);

#ifdef TRACE_DEBUG
  if (DEBUGGING_ON)
    db_printf("\n>>>stack_decode(addr=%x, stack=%x, task=%x)\n", addr, *stack,
              tk);
#endif
  ...
  if (*who_is_task(tk) == 'U')
    ...
    proc = db_search_task_symbol(addr, DB_STGY_PROC, &offset_from_proc, tk);
  ...
  if (proc != DB_SYM_NULL) {
    char *names;
    db_symbol_values((db_symtab_t *)0, proc, &names, &function_addr);
    if (names == 0)
      return 0;
#ifdef TRACE_DEBUG
    if (DEBUGGING_ON)
    ...
    for (check_addr = addr; instructions_to_check-- > 0; check_addr -= 4) {
      if (!db_trace_get_val(check_addr, &inst, tk))
        break;
      ...
      if (SUBU_R31_R31_IMM(inst)) {
		     ...
		     * a function.
		     */
		    if (!db_trace_get_val(check_addr + 4, &inst, tk))
			continue;
  ...
  if (!db_trace_get_val(function_addr, &inst, tk)) {
#ifdef TRACE_DEBUG
    if (DEBUGGING_ON)
  ...
  *stack += IMM16VAL(inst);
  ...
  for (instructions_to_search = (addr - check_addr) / sizeof(long);
    ...
    if (!db_trace_get_val(check_addr, &instruction.rawbits, tk)) {
#ifdef TRACE_DEBUG
      if (DEBUGGING_ON)
        db_printf("couldn't read %x at line %d\n", check_addr, __LINE__);
#endif
    ...
    if ((flags & STORE) && instruction.diatic.s1 == /*stack pointer*/ 31) {
      ...
      if (!have_local_reg(instruction.diatic.d)) {
        ...
        if (db_trace_get_val(r31 + instruction.diatic.i16, &value, tk))
      ...
      if ((flags & DOUBLE) && !have_local_reg(instruction.diatic.d + 1)) {
        ...
        if (db_trace_get_val(r31 + instruction.diatic.i16 + 4, &value, tk))
  ...
  if (ret_addr & 1) {
    ...
    switch (is_jump_source_ok(ret_addr, function_addr, tk)) {
...
static void db_stack_trace_cmd2(db_regs_t *regs, task_t task) {
  unsigned stack;
  unsigned depth = 1;
  unsigned where;
  unsigned ft;
  ...
  tk = (ft == 2 ? task : kernel_task /* TASK_NULL */);
  ...
  if (trace_flags & TRACE_SHOWADDRESS_FLAG)
    db_printf("%08x ", where);
  db_task_printsym(where, DB_STGY_PROC, tk);
  clear_global_saved_regs();

  /* see if this routine had a stack frame */
  if ((where = stack_decode(where, &stack, tk)) == 0) {
    where = regs->r[1];
    db_printf("(stackless)");
  } else {
    print_args();
  ...
  do {
    ...
    if (trace_flags & TRACE_SHOWADDRESS_FLAG)
      db_printf("%08x ", where);
    db_task_printsym(where, DB_STGY_PROC, tk);
    where = stack_decode(where, &stack, tk);
    print_args();
    if (trace_flags & TRACE_SHOWFRAME_FLAG)
      db_printf(" [frame 0x%x]", stack);
    db_printf("\n");
  ...
  while (i) {
     ...
     * will just have the address of the frame (off in the thread pcb)
     * replicated twice.
     *
    ...
    db_read_bytes((char *)stack, 2 * sizeof(int), (char *)pair, kernel_task);
    ...
    if (pair[0] == pair[1]) {
      if (pair[0] != stack + 8) {
        ...
        */
        ...
        db_printf("-------------- %s [EF: 0x%x] -------------\n", cause, frame);
        db_stack_trace_cmd2(frame, task);
        return;
      }
#ifdef TRACE_DEBUG
  ...
  if (trace_flags & TRACE_USER_FLAG) {
    ...
    if (badwordaddr((vm_offset_t)stack) || badwordaddr((vm_offset_t)stack))
      return;
    ...
    if (frame_is_sane(user) == 2) {
      ...
      db_stack_trace_cmd2(user, task);
    }
  }
}

/*
 * stack trace - needs a pointer to a luna88k saved state. If none is given,
 * assume the current cpu. If given a number between 0 and 3, it traces
 * that cpu.
 *
 * If argument t is given, it traces that thread starting from its
 * saved pcb state.
 *
 * If argument f is given, the stack pointer of each call frame is
 * printed.
 */
void db_stack_trace_cmd(db_regs_t *addr, int have_addr, db_expr_t count,
                        char *modif) {
  enum { Default, CPU, Thread, Stack, Frame } style = Default;
  task_t tk = kernel_task;
  ...
  while (modif && *modif)
    ...
    case 'h':
      ...
      db_printf("  t = ARG is thread (/t is default if ARG given)\n");
      db_printf("  s = ARG is a stack pointer\n");
      db_printf("  f = ARG is a frame pointer\n");
      db_printf("  c = ARG is a cpu number.\n");
#ifdef TRACE_DEBUG
  ...
  if (!have_addr && style != Default) {
    db_printf("expecting argument with /s, /t, /c, or /f\n");
    return;
  }
  if (have_addr && style == Default)
    style = Thread;
  ...
  case CPU:
    if (arg.num > MAX_CPUS) {
      db_printf("CPU number (%d) too large.\n", arg.num);
      return;
    }
    regs = db_cur_exc_frame[arg.num];
    break;

  case Default:
    if (current_thread())
      tk = current_thread()->task;
    else
      tk = TASK_NULL;
    regs = DDB_REGS;
    break;
  ...
  case Thread: {
    ...
    if (badwordaddr((vm_offset_t)arg.thread) ||
        badwordaddr((vm_offset_t)&arg.thread->swap_func) ||
        badwordaddr((vm_offset_t)&arg.thread->pcb->kernel_state)) {
      db_printf("bad thread pointer given to stack trace cmd\n");
      return;
    }

    /*
     * if the thread is blocked with a continuation,
     * we can only do a user space trace
     */
    if ((arg.thread)->swap_func) /* blocked with a continuation */
    {
      db_printf("thread 0x%x blocked with continuation 0x%x; ", arg.thread,
                arg.thread->swap_func);
      if (trace_flags & TRACE_USER_FLAG) {
        regs = &((arg.thread)->pcb->user_state);
        db_printf("only tracing user space\n");
        db_printf("---------------- %s [EF : 0x%x] -------------\n",
                  m88k_exception_name(regs->vector), regs);
      } else {
        db_printf("trace aborted\n");
        return;
      }
    } else {
      bzero((void *)&frame, sizeof(frame));
      /* translate from saved state into exception frame format */
      p = &(arg.thread->pcb->kernel_state);
      frame.epsr = 0x800003f0U;
      frame.sxip = p->pcb_pc | 2;
      frame.snip = frame.sxip + 4;
      frame.sfip = frame.sfip + 4;
      /* snag r14 - r30 */
      bcopy((void *)&(p->pcb_r14), &(frame.r[14]), sizeof(int) * (30 - 14 + 1));
      /* stack pointer */
      frame.r[31] = p->pcb_sp;
      regs = &frame;
    }
...
#define REASONABLE_FRAME_DISTANCE 2048
    ...
    for (ptr = arg.num; /**/; ptr += 4) {
      ...
      if (db_trace_get_val(ptr, &val1, tk) == 0) {
       ...
       */
      if (val1 <= ptr || (val1 & 3) || val1 > (ptr + REASONABLE_FRAME_DISTANCE))
        continue;

      /* peek at the next word to see if it could be a return address */
      if (db_trace_get_val(ptr, &sxip, tk) == 0) {
        db_printf("can't read from %x, aborting.\n", ptr);
        return;
      }
      if (sxip == 0 || !db_trace_get_val(sxip, &val2, tk))
        continue;

      if (db_trace_get_val(val1, &val2, tk) == 0) {
        db_printf("can't read from %x, aborting.\n", val1);
        continue;
      }
      ...
      if (
          ...
          val2 == 0x12345678
#endif
          && db_trace_get_val(val1 - 4, &val2, tk) && val2 == val1 &&
          db_trace_get_val(val1 - 8, &val2, tk) && val2 == val1) {
...
void diddle_rom_colors(void)
{
     unsigned *addr = (unsigned *)RVPtr->EPundisp;
     union instruction inst;
     unsigned (*addr_of_getvbr)();
     unsigned addr_of_clearscreen;
     unsigned reg, count, offset;

     /*
      * The 1st bsr in will be to getvbr(), the 2nd to clearscreen.
      *	Find them.
      */
    while ((inst.rawbits = *addr) &&
	   (m88k_instruction_info(inst.rawbits)&BSR) == 0) {
		addr++;
    }
    addr_of_getvbr = (void*)br_dest((unsigned)addr++, inst);

    while ((inst.rawbits = *addr) &&
	   (m88k_instruction_info(inst.rawbits)&BSR) == 0) {
		addr++;
    }
    addr_of_clearscreen = br_dest((unsigned)addr, inst);

    /*
     * Now look in clearscreen.  The first bsr will be to getvbr.
     * See what's added to and loaded from the result... we want to 
     * get the address of the halfword that's loaded.
     */
    addr = (unsigned *)addr_of_clearscreen;
    while ((inst.rawbits = *addr) &&
	   (m88k_instruction_info(inst.rawbits) & BSR) == 0) {
		addr++;
    }
    if (br_dest((unsigned)addr, inst) != (unsigned)addr_of_getvbr) {
	db_printf("[strangeness... first branch of clearscreen [%x] "
	    "not to %x\n", br_dest((unsigned)addr, inst), addr_of_getvbr);
	/* return */
    }
    addr += inst.br.n ? 2 : 1;

    /*
     * Now follow a limited number of instructions.
     * The address that will be manipulated is in R2.
     */
#define INSTRUCTIONS_TO_FOLLOW 20
    reg = 2;
    offset = 0;
    count = 0;
    while (count++ < INSTRUCTIONS_TO_FOLLOW) {
	 inst.rawbits = *addr;
	addr++;

	/* see if it's an OR to move registers */
	if ((inst.rawbits & 0xfc00fe00U) == 0xf4005800U) {
	    if (inst.triatic.s1 == 0 && inst.triatic.s2 == reg)
		reg = inst.triatic.d;
	    else if (inst.triatic.s1 == reg && inst.triatic.s2 == 0)
		reg = inst.triatic.d;
	    continue;
	}

	/* see if it's an ADDU (imm16) off of our registsr */
	if ((inst.rawbits & 0xfc000000u) == 0x60000000U) {
	    if (inst.diatic.s1 == reg) {
		offset += inst.diatic.i16;
		reg = inst.diatic.d;
	    }
	    continue;
	}

	/* see if it's a ld.h off of our register */
	if ((inst.rawbits & 0xec000000U) == 0x08000000) {
	     if (inst.diatic.s1 == reg) {
		unsigned short *diddle;
		offset += inst.diatic.i16;
		/*
		 * Found it!
		 * We now have the address of the halfword to diddle
		 * as  getvbr()+offset.
		 */
		diddle = (unsigned short*)((*addr_of_getvbr)() + offset);
		*diddle = 0xffff;
		return;
	     }
	}
    }
    db_printf("couldn't diddle... so sad\n");
}
#endif
