 ...
 * HISTORY
 * $Log:	trap.c,v $
 * Revision 2.15  93/11/17  17:36:53  dbg
 * 	Removed interrupted_pc: it is now a macro in machine/thread.h.
 * 	[93/10/14            dbg]
 *
 ...
 * 	Some reorganization. Better error messages for debugging.
 * 	Added error_fault().
 * 	[92/12/07            jfriedl]
 *
...
#include <kern/exception.h> /* exception() */
 ...
 *	occurs.
 */
no_return user_page_fault_continue(kern_return_t kr) {
  struct m88100_saved_state *frame = &current_thread()->pcb->user_state;

  if (kr == KERN_SUCCESS) /* all went well */
  {
#if MACH_KDB
    if (db_watchpoints_enabled() && (frame->dmt0 & DMT_VALID) &&
...

  {
    ...
    DEBUG_MSG("[%s FAULT cpu=%d th=%x t=%x R=%x pipe:t=%x/d=%x/a=%x]\n", who,
              cpu_number(), current_thread(), task, frame, frame->dmt0,
              frame->dmd0, frame->dma0);
#if MACH_KDB
  ...
  /*
   *	Pass faulting address as 'subcode' for exception
   */
  exception(EXC_BAD_ACCESS, kr,
            (frame->vector == EVN_INS) ? frame->sxip & XIP_ADDR : frame->dma0);
}
...
#endif
{
  ...
  if (FROM_SYSTEMMODE(frame->epsr)) {
    ...
    if (type == T_INSTFLT || type == T_DATAFLT) {
      ...
      XPR(XPR_VM_FAULT, ("vm_fault(%x,%x) frame=%x pc=%x th=%x\n", map,
                         fault_addr, frame, frame->sxip, current_thread()));

      result = vm_fault(map, trunc_page((vm_offset_t)fault_addr), protection,
                        FALSE, FALSE, 0);
      ...
      if (recover == 0) {
        extern unsigned ALLOW_FAULT_START, ALLOW_FAULT_END, FAULT_ERROR;
    ...
    } else if (frame->vector == ENV_MISALIGN) /* misaligned access exception*/
    {
      ...
      DEBUG_MSG("(CPU %d) trap type %d, vector=%d,", mycpu, type,
                frame->vector);
  ...
  user_fault:
    ...
    case T_DATAFLT: {
      ...
      vm_fault_noreturn(current_thread()->task->map, trunc_page(fault_addr),
    ...
    default:
      ...
      exc_subcode = type;
      DEBUG_MSG(
          "[BAD USER TRAP cpu %d type=%d, vector=%d, sxip=%x, frame=%x]\n",
    ...
    if (exc_type == EXC_BAD_INSTRUCTION || exc_type == EXC_BAD_ACCESS ||
        exc_type == EXC_SOFTWARE) {
      char *who = who_is_task(current_task());
      DEBUG_MSG("[cpu %d exception(%d,%d,%d) to %s thread=%x frame=%x]\n",
                mycpu, exc_type, exc_code, exc_subcode, who, current_thread(),
                frame);
...
#define STACK_MARGIN 1024
  ...
  for (i = 0; i < 4; i++) {
    static x = 0;
    if (frame > active_stacks[i] &&
        frame < active_stacks[i] + KERNEL_STACK_SIZE) {
      if (x == 0 && i != cpu_number()) {
      ...
      if (frame < active_stacks[i] + STACK_MARGIN) {
        raw_printf(
            "[kernel stack redline hit! cpu=%d,frame=%x,thread=%x,task=%x]\n",
...
#if defined(TRAPTRACE)
  if (traptrace != 0 &&
      (traptrace == 0xffffffffU || traptrace == (unsigned)current_thread() ||
       traptrace == (unsigned)current_task())) {
    DEBUG_MSG("[TRAP TRACE cpu=%d thread=%x type=%d%s frame=%x "
              "(xip=%x, r1=%x, r31=%x)]\n",
              cpu_number(), current_thread(), type,
              type == T_INSTFLT ? "(I-fault)"
