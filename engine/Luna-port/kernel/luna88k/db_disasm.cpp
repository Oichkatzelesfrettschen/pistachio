 ...
 * HISTORY
 * $Log:	db_disasm.c,v $
 * Revision 2.9  93/01/26  18:02:45  danner
 * 	ANSIfied. Cleaned up includes.
 * 	[93/01/24            jfriedl]
 *
 * Revision 2.8  93/01/14  17:37:53  danner
 ...
 * 	[91/04/04            rvb]
 *
 * 	 Modifed symofset to use db_printsym.
 * 	[91/03/14            danner]
 *
 * 		Created from luna/kdb/das.c
 * 	[90/11/30            danner]
 *
 */

/*
 * luna88k disassembler for use in ddb
 */

#include <ddb/db_access.h> /* db_get_task_value() */
#include <ddb/db_output.h> /* db_printf() */
#include <ddb/db_sym.h>    /* DB_STGY_PROC, db_task_printsym() */
#include <machine/db_machdep.h>

static task_t db_disasm_task;

static char *instwidth[4] = {".d", "  ", ".h", ".b"};

static char *condname[6] = {"gt0 ", "eq0 ", "ge0 ", "lt0 ", "ne0 ", "le0 "};

static char *ctrlreg[64] = {"cr0(PID)   ",
                            "cr1(PSR)   ",
                            "cr2(EPSR)  ",
                            "cr3(SSBR)  ",
                            "cr4(SXIP)  ",
...
static void oimmed(long inst, char *opcode, long iadr) {
...
static void ctrlregs(long inst, char *opcode, long iadr) {
...
static void sindou(int inst, char *opcode, long iadr) {
...
static void jump(long inst, char *opcode, long iadr) {
  register int rs2 = inst & 037;
...
static void instset(long inst, char *opcode, long iadr) {
  register int rs2 = inst & 037;
  register int rs1 = (inst >> 16) & 037;
  register int rd = (inst >> 21) & 037;
...
static void symofset(int disp, int bit, int iadr) {
  long addr;
  ...
  db_task_printsym(addr, DB_STGY_PROC, db_disasm_task);
  return;
}

static void obranch(int inst, char *opcode, long iadr) {
  int cond = (inst >> 26) & 01;
...
static void brcond(int inst, char *opcode, long iadr) {
  int cond = (inst >> 26) & 1;
...
static void otrap(int inst, char *opcode, long iadr) {
  int vecno = inst & 0777;
  int match = (inst >> 21) & 037;
  int rs = (inst >> 16) & 037;
...
static void obit(int inst, char *opcode, long iadr) {
  int rs = (inst >> 16) & 037;
  int rd = (inst >> 21) & 037;
...
static void bitman(int inst, char *opcode, long iadr) {
...
static void immem(int inst, char *opcode, long iadr) {
  ...
  db_printf("\t%s%s%c\t\tr%-3d,r%-3d,", opcode, instwidth[aryno], c, rd, rs);
  printval(immed);
}

/* Handles triadic mode load/store/exchange instructions */
static void nimmem(int inst, char *opcode, long iadr) {
  ...
  if (user_bit && signed_fg && (aryno == 01)) {
    if (st_lda)
      ...
      db_printf("\t%s%s\tr%-3d,r%-3d", opcode, user, rd, rs1);
  } else if (user_bit && signed_fg)
    db_printf("\t%s%s%s\tr%-3d,r%-3d", opcode, instwidth[aryno], user, rd, rs1);
  else
    db_printf("\t%s%s%c%s\tr%-3d,r%-3d", opcode, instwidth[aryno], c, user, rd,
              rs1);
...
static void lognim(int inst, char *opcode, long iadr) {
  register int rd = (inst >> 21) & 037;
  register int rs1 = (inst >> 16) & 037;
  register int rs2 = inst & 037;
...
static void onimmed(int inst, char *opcode, long iadr) {
  register int rd = (inst >> 21) & 037;
  register int rs1 = (inst >> 16) & 037;
  register int rs2 = inst & 037;
...
db_addr_t db_disasm(db_addr_t loc, boolean_t altfmt, task_t task) {
  db_disasm_task = task;
  luna_print_instruction(loc, db_get_task_value(loc, 4, 0, task));
  db_printf("\n");
  return loc + 4;
}
