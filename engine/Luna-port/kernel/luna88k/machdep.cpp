 ...
 * HISTORY
 * $Log:	machdep.c,v $
 * Revision 2.11  93/11/17  17:36:32  dbg
 * 	Include <kern/machine.h>.  Remove setsoftclock (no longer
 * 	used).  Call sys_clock_interrupt instead of clock_interrupt.
 * 	[93/10/15            dbg]
 *
...
static unsigned int int_mask_val[INT_LEVEL] = {
    INT_SET_LV0, INT_SET_LV1, INT_SET_LV2, INT_SET_LV3,
    INT_SET_LV4, INT_SET_LV5, INT_SET_LV6, INT_SET_LV7};

/* static */ int clock_enabled[MAX_CPUS];

static volatile unsigned int *clock_reg[MAX_CPUS] = {
    (volatile unsigned int *)OBIO_CLOCK0, (volatile unsigned int *)OBIO_CLOCK1,
    (volatile unsigned int *)OBIO_CLOCK2, (volatile unsigned int *)OBIO_CLOCK3};

#if !MACH_KDB
static
#endif
    volatile unsigned int *soft_reg[MAX_CPUS] = {
        (volatile unsigned int *)SOFT_INT0, (volatile unsigned int *)SOFT_INT1,
        (volatile unsigned int *)SOFT_INT2, (volatile unsigned int *)SOFT_INT3};
...
static void level4_intr(void) { se_intr(0, /* dummy*/ 0); /* lance */ }

void level3_intr(void) { spc_intr(0); }
...
static void (*scb_level4[SAME_LEVEL_DEVICES])() = {level4_intr, 0, 0, 0, 0};

static void (*scb_level5[SAME_LEVEL_DEVICES])() = {level5_inter, 0, 0, 0, 0};

static void (*scb_level6[SAME_LEVEL_DEVICES])();

static void (**scb_device[INT_LEVEL + 1])() = {
    scb_level0, scb_level1, scb_level2, scb_level3,
    scb_level4, scb_level5, scb_level6, 0};

/*
 * Machine-dependent startup code
 */
void startup(vm_offset_t firstaddr) { gimmeabreak(); }
 ...
 */
#if MACH_KDB
unsigned db_spl(void) { return *int_mask_reg[cpu_number()] << 8; }
#endif
...
static int spl_level_table[64] = {
    I6, I0, I1, I0, I2, I0, I1, I0, I3, I0, I1, I0, I2, I0, I1, I0,
    I4, I0, I1, I0, I2, I0, I1, I0, I3, I0, I1, I0, I2, I0, I1, I0,
    I5, I0, I1, I0, I2, I0, I1, I0, I3, I0, I1, I0, I2, I0, I1, I0,
    I4, I0, I1, I0, I2, I0, I1, I0, I3, I0, I1, I0, I2, I0, I1, I0};
...
void start_clock(void) { clock_enabled[cpu_number()] = 1; }
 ...
 */
static void ext_int(int *eframe, int vec, int level, int cont_code) {
  register int cpu = cpu_number();
...
int_done:
  if (part2 == EI_SKIP) {
    ...
    *int_mask_reg[cpu] = eframe[EF_MASK];
    ...
    ext_int(eframe, vec, level, part2);
  }
}
...
kern_return_t cpu_start(int slot_num) { return KERN_FAILURE; }

kern_return_t cpu_control(int slot_num, processor_info_t info,
                          natural_t count) {
  return KERN_FAILURE;
}
 ...
 */
vm_offset_t phystokv(vm_offset_t phys) { return phys + VEQR_ADDR; }
 ...
 */
void bm_align_screen(void) { *(volatile unsigned *)BMAP_RFCNT = 0x0007ffe5; }
 ...
 */
void bm_set_palette_colors(unsigned char old[2 /*fg and bg*/][3 /*r g b*/],
                           unsigned fg_R, unsigned fg_G, unsigned fg_B,
                           unsigned bg_R, unsigned bg_G, unsigned bg_B) {
