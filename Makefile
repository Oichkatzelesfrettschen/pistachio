...
build/tests/posix/test_signal: tests/posix/test_signal.cc user/lib/posix/posix.cc | build/tests/posix
	$(CXX) $(CXXFLAGS) -Iuser/lib/posix $^ -o $@

tests: build/tests/spinlock_fairness build/tests/posix/test_file build/tests/posix/test_process build/tests/posix/test_signal
