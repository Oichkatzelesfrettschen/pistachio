...
constexpr std::array<std::uint16_t, 26> icode{
    0x2E7C,
    HIGH,
    static_cast<std::uint16_t>(LOW + 0x100),
    0x227C,
    HIGH,
    static_cast<std::uint16_t>(LOW + 0x26),
    0x223C,
    HIGH,
    static_cast<std::uint16_t>(LOW + 0x22),
    0x207C,
    HIGH,
    static_cast<std::uint16_t>(LOW + 0x2A),
    0x42A7,
    0x303C,
    0x003B,
    0x4E40,
...
[[maybe_unused]] void copyseg(int from, int to) {
  ...
  std::memmove(reinterpret_cast<void *>(to * click),
               reinterpret_cast<const void *>(from * click), click);
}
