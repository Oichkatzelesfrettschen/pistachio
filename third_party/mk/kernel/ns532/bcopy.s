...
/*
 * HISTORY
 * 09-Sep-92  Ian Dall (idall) at University of Adelaide
 *	ifdef'ed out bcopy_bytes. It doesn't work and we now have a version
 *	written in C.
 *
 * 20-Aug-92  Ian Dall (ian) at University of Adelaide
 *	Fix addressing modes for accessing bcopy_bytes_failed and
 *	bcopy_bytes_done in bcopy_bytes.
 *
 * 17-Aug-92  Ian Dall (ian) at University of Adelaide
 *	Added global variable bcopy_bytes_done. Without it, it very
 *	difficult to distinguish between an interrupt before bcopy_bytes
 *	has started and after bcopy_bytes has finished. We need to know
 *	so we know the number of bytes actually transferred.
 *
...
LEX(bcopy_bytes_failed)
	.long 0
	.globl EX(bcopy_bytes_done)
LEX(bcopy_bytes_done)
	.long 0
