...
/*
 * HISTORY
 * 03-Dec-92  Ian Dall (ian) at University of Adelaide
 *	Some of the loops with ptes_per_vm_page don't increment the pte
 *	pointer. This shouldn't matter since ptes_per_vm_page is always 1
 *	in the current implimentation. Still, might as well get it right!
 *
 * 02-Dec-92  Ian Dall (ian) at University of Adelaide
 *	NS532_PTE_MOD is too big to fit in a char so right shift
 *	NS532_PTE_MOD and NS532_PTE_REF to get PHYS_MOD and PHYS_REF. New
 *	macro PTE_TO_PHYS.
 *
 ...
 *
 * $Log: pmap.c,v $
 * Revision 1.18  1992/05/12  22:33:20  jvh
 * 	Cleanup.
 *
 * Revision 1.15  1992/04/11  00:06:13  jvh
 * 	pmap_bootstrap now works together with physical_mode.c.
 *	Avail_start and avail_end are passed in from physical_mode.c
 *	through ns532_init().
 *
 * Revision 1.12  1991/07/15  08:34:22  jvh
 * 	Removed unused variables in pmap_attributes().
 * 	Changed lockings to PMAP_WRITE_LOCK in pmap_attributes().
 *
 * Revision 1.10  91/07/15  08:08:50  jvh
 * 	Added pmap_attribute(). It exports cache flushing to user.
 * 	Cache inhibiting per page could be added if there is a need for it.
 * 
 * Revision 1.9  91/07/15  07:41:46  jvh
 * 	Changed pmap interfaces to correspond to MK55 changes.
 * 	Removed splvm() and splx() from SPLVM and SPLX macros in
 * 	single CPU case.
 * 	Added comment in pmap_map_bd noting wraparound.
 * 	Fixed pmap_bootstrap.
 * 
 * Revision 1.6  91/04/15  03:21:26  jvh
 * 	 Removed some incorrect (?) assertions.
 * 	 Added continuation argument to VM_PAGE_WAIT.
 * 
 * Revision 1.5  91/04/15  03:17:16  jvh
 * 	Upgraded to new kmem_alloc interface.
 * 
 * Revision 1.3  91/03/30  20:14:24  jvh
 * 	Added relevant MK42 changes. Made them conditional (#ifdef MK30).
 * 
 * Revision 1.1  91/01/26  18:43:47  ylo
 * 	Initial revision.
 */
/*
 * 	File: ns532/pmap.c
 *	32532 changes: Tatu Ylonen. Johannes Helander
 ...
 *
 *	Physical Map management code for ns532.
 *
 ...
 */
#define PTE_TO_PHYS(pte) ((pte)>>1)
#define	PHYS_MODIFIED	PTE_TO_PHYS(NS532_PTE_MOD)	/* page modified */
#define	PHYS_REFERENCED	PTE_TO_PHYS(NS532_PTE_REF)	/* page referenced */
...
int		ptes_per_vm_page;	/* number of 32532 ptes needed
					   to map one VM page. */
	...
	register vm_offset_t	addr;
{
	...
	pte = pmap->ptb[pdenum(addr)];

	if ((pte & NS532_PTE_VALID) == 0)
		return(PT_ENTRY_NULL);
...
#define	pmap_pde(pmap, addr) (&(pmap)->ptb[pdenum(addr)])
	...
	ptep_t	ptep;
{
	...
	epte = pte + NS532_PGBYTES/sizeof(pt_entry_t);
	ctu = 0;
	ctw = 0;
	while (pte < epte) {
		if (pte->pfn != 0) {
 ...
 *
 *	This sets the cache inhibit bit on the pages. (ns532)
 */
	...
	vm_prot_t		prot;
{
	...
	register pt_entry_t	*pte;
	register vm_offset_t    len;

	template = pa_to_pte(start)
		| NS532_PTE_VALID | NS532_PTE_CI;
	if (prot & VM_PROT_WRITE)
	    template |= NS532_PTE_WRITE;

	/* 0xfffff000 + 0x1000 == 0 !!! */
	/* We need to map the last page of memory. Take care of wraparound. */
	for (len = end - start; len > 0; len -= PAGE_SIZE) {
 ...
 *	Called with mapping ON (on the 532).  Page_size must already be set.
 *
 *	Parameters:
...
vm_size_t morevm = 28 * 1024 * 1024;	/* VM space for kernel map */
					/* Max that can be alloced by io_map */
	...
	vm_offset_t	load_start;
{
	 ...
	 */
	ptes_per_vm_page = page_size / NS532_PGBYTES;
	 ...
	 *
	 * We did these mappings in physical_mode.c
	 */

/*
 *	startup requires additional virtual memory (for tables, buffers, 
 *	etc.).  Drivers may also require some of that memory.
 */
	/* allocate N Page Tables */
	npt = morevm >> PDESHIFT;
	pa_ptep = avail_start;
	avail_start += ns532_ptob(npt);

	printf("pmap_bootstrap: zeroing %x pages at %x to map morevm\n",
	       npt, phystokv(pa_ptep));
	/* zero the new page tables */
	bzero(phystokv(pa_ptep), ns532_ptob(npt));

	/* 
	 * virtual_avail and virtual_end are now initialized in
	 * ns532_init from values passed in from physical_mode.c
	 */

	/* round to next pde (4 MB "page") */
	virtual_end = (virtual_end + ((1 << PDESHIFT) - 1)) 
	    & ~((1 << PDESHIFT) - 1);

	/* enter new page tables to kpde */
	for (i = 0; i < npt; i++) {
		kpde[pdenum(virtual_end)] = pa_to_pte(pa_ptep) 
		    | NS532_PTE_VALID | NS532_PTE_WRITE;
		pa_ptep += NS532_PGBYTES;
		virtual_end += 1 << PDESHIFT; /* each pde maps 4 MB */
	}
#if 0
	printf("pmap_bootstrap: avail_end=%x\n", avail_end);

	printf("pmap_bootstrap: virtual_avail=%x\n", virtual_avail);
	printf("pmap_bootstrap: virtual_end=%x\n", virtual_end);
#endif

/*
 *	Invalidate virtual addresses at 0.
 *	The 1-1 low memory mapping is destroyed.
 *	The PT still exists and can be reused on shutdown.
 */
	kpde[0] = 0;

	kernel_pmap->ptb = kpde;
}

/* 
 * pmap_attribute: Flush cache, get/set cache inhibit bit. Exported.
 */
	...
	vm_machine_attribute_val_t* value;		/* IN/OUT */
{
	...
	case MATTR_VAL_GET:		/* (generic) return current value */
	  	/* Cache inhibit. Not implemented. */
	  	ret = KERN_INVALID_ARGUMENT;
		break;

	/* Flush cache. Ignore addresses and flush the entire cache */
 ...
 */
	...
	if (kmem_alloc_wired(kernel_map,
			     (vm_offset_t *)&p->ptb, NS532_PGBYTES)
	    != KERN_SUCCESS)
	  panic("pmap_create");

	bcopy(kpde, p->ptb, NS532_PGBYTES);
	p->ref_count = 1;
	...
	register pmap_t	p;
{
	 ...
	 */
	for (pdep = p->ptb;
	     pdep < &p->ptb[pdenum(VM_MIN_KERNEL_ADDRESS)];
	     pdep += ptes_per_vm_page) {
	    if (*pdep & NS532_PTE_VALID) {
		pa = pte_to_pa(*pdep);
	...
	kmem_free(kernel_map, p->ptb, NS532_PGBYTES);
...
/*
 *	Remove a range of 32532 page-table entries.
	...
	pt_entry_t		*epte;
{
	...
	for (cpte = spte; cpte < epte;
		...
		do {
			...
			PTE_TO_PHYS(*lpte) & (PHYS_MODIFIED|PHYS_REFERENCED);
		    *lpte = 0;
		    lpte++;
		} while (--i > 0);
	    }
 ...
 *	rounded to the 32532 page size.
 */
	...
	vm_offset_t	s, e;
{
	...
	while (s < e) {
	    ...
	    if (l > e)
		l = e;
	    if (*pde & NS532_PTE_VALID) {
		spte = (pt_entry_t *)ptetokv(*pde);
		spte = &spte[ptenum(s)];
		epte = &spte[ns532_btop(l-s)];
	...
	vm_prot_t	prot;
{
	...
	if (pv_h->pmap != PMAP_NULL) {
...

		{
		     ...
		     * Consistency checks.
		     */
		    /* assert(*pte & NS532_PTE_VALID); XXX */
		...
		if (remove || pmap == kernel_pmap) {
			...
			do {
			    pmap_phys_attributes[pai] |=
				PTE_TO_PHYS(*pte) & (PHYS_MODIFIED|PHYS_REFERENCED);
			    *pte++ = 0;
			} while (--i > 0);
		    }
	...
	vm_prot_t	prot;
{
	...
	while (s < e) {
		...
		while (spte < epte) {
		    if (*spte & NS532_PTE_VALID) {
			*spte &= ~NS532_PTE_WRITE;
		      }
		    spte++;
		}
	    }
...
Retry:
	 ...
	 *	pmap is always expanded to include enough 532
	...
	if (old_pa == pa) {
	     ...
	     */
	    if (wired && !(*pte & NS532_PTE_WIRED))
		pmap->stats.wired_count++;
	    else if (!wired && (*pte & NS532_PTE_WIRED))
		pmap->stats.wired_count--;

	    template = pa_to_pte(pa) | NS532_PTE_VALID;
	    if (pmap != kernel_pmap)
	      template |= NS532_PTE_USER;
	    if (prot & VM_PROT_WRITE)
	      template |= NS532_PTE_WRITE;

	    if (wired)
		template |= NS532_PTE_WIRED;
	    PMAP_UPDATE_TLBS(pmap, v, v + PAGE_SIZE);
	    i = ptes_per_vm_page;
	    do {
		if (*pte & NS532_PTE_MOD)
		    template |= NS532_PTE_MOD;
	...
	vm_offset_t	va;
{
	...
	if ((pte = pmap_pte(pmap, va)) == PT_ENTRY_NULL)
	    pa = (vm_offset_t) 0;
	else if (!(*pte & NS532_PTE_VALID))
	    pa = (vm_offset_t) 0;
	else
	    pa = pte_to_pa(*pte) + (va & NS532_OFFMASK);
	simple_unlock(&pmap->lock);
	SPLX(spl);
	return(pa);
}
	...
	register vm_offset_t	v;
{
	 ...
	 *	If we have allocated more than one 532 page,
	 *	set several page directory entries.
	 */
	PMAP_READ_LOCK(map, spl);
	i = ptes_per_vm_page;
	pdp = &map->ptb[pdenum(v) & ~(i-1)];
	do {
	    *pdp = pa_to_pte(pa)
		| NS532_PTE_VALID
		| NS532_PTE_USER
	...
	pmap_t 		p;
{
	...
	for (pdp = p->ptb;
	     pdp < &p->ptb[pdenum(VM_MIN_KERNEL_ADDRESS)];
	     pdp += ptes_per_vm_page)
	{
	    if (*pdp & NS532_PTE_VALID) {
	...
	vm_offset_t	src, dst;
{
	...
	while (i--) {
		copy_phys(ns532_pfn(src), ns532_pfn(dst));
		src += NS532_PGBYTES;
		dst += NS532_PGBYTES;
	}
}
