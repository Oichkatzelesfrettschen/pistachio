 ...
 * HISTORY
 * $Log:	maxine.c,v $
 * Revision 2.12  93/11/17  17:51:29  dbg
	...
	spl_t spllevel)
{
	...
	/*
	 *	Don`t use the free-running counter for clock time...
	 */
 ...
 *
 */
void xine_slot_hand_fill(
	tc_option_t	*slot)
{
	register int	i;

	for (i = XINE_FLOPPY_SLOT; i < XINE_CLOCK_SLOT+1; i++) {
		slot[i].present = 1;
		slot[i].slot_size = 1;
		slot[i].rom_width = 1;
		slot[i].isa_ctlr = 0;
	...
	/* time-of-year clock */
	bcopy("MC146818", slot[XINE_CLOCK_SLOT].module_name, TC_ROM_LLEN+1);
	slot[XINE_CLOCK_SLOT].k1seg_address = PHYS_TO_K1SEG(XINE_SYS_CLOCK);
