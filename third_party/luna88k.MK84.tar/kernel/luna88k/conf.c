 ...
 * HISTORY
 * $Log:	conf.c,v $
 * Revision 2.13  93/11/17  17:35:14  dbg
 * 	Added clock device.
 * 	[93/06/09            dbg]
 * 
...
/* Clock */
extern int clk_open(), clk_close(),
	   clk_getstat(), clk_setstat(), clk_info();
extern vm_offset_t	clk_mmap();

#define	clk_name	"clock"
#define	clk_priv_name	"clock_priv"
...
struct dev_ops dev_name_list[] =
{
	...
	{ clk_priv_name,clk_open,	clk_close,	nodev,
	  nodev,	clk_getstat,	clk_setstat,	clk_mmap,
	  nodev,	nodev,		nodev,		0,
	  clk_info },

	{ clk_name,	clk_open,	clk_close,	nodev,
	  nodev,	clk_getstat,	nodev,		clk_mmap,
	  nodev,	nodev,		nodev,		0,
	  clk_info },
