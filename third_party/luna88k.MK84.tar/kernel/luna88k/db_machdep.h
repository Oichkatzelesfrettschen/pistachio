 ...
 * HISTORY
 * $Log:	db_machdep.h,v $
 * Revision 2.11  93/11/17  17:35:43  dbg
 * 	Added 'extern' to function prototypes.  Removed prototypes for
...
extern db_regs_t      *db_cur_exc_frame[NCPUS];	/* register state for
						   debugger */
 ...
 */
extern int luna_print_instruction(unsigned iadr, long inst);
extern db_addr_t db_disasm(db_addr_t loc, boolean_t altfmt, task_t task);

/*
 * Prototypes, etc. from "luna88k/db_interface.c"
 */
extern int db_getc(void);
extern void db_putc(int c);
extern int luna_pc(struct m88100_saved_state *regs);
extern void ddb_error_trap(char *error, db_regs_t *eframe);
extern int ddb_entry_trap(int level, db_regs_t *eframe);
extern int ddb_break_trap(int type, db_regs_t *eframe);
extern int ddb_softcall_trap(int level, db_regs_t *eframe);
extern int ddb_nmi_trap(int level, db_regs_t *eframe);
extern void gimmeabreak(void);
extern void db_printing(void);
extern boolean_t db_check_user_addr(vm_offset_t va);
extern boolean_t
	db_phy_eq(task_t t1, vm_offset_t va1, task_t t2, vm_offset_t va2);
extern boolean_t
	db_check_access(task_t task, vm_offset_t va, unsigned int size);
extern void cpu_interrupt_to_db(int cpu_no);
extern void kdb_kintr(void);
extern struct db_command db_machine_cmds[];
extern void kdb_init(void);
extern char *db_task_name(task_t task);

/*
 * Prototypes from "luna88k/db_sstep.c"
 */
extern boolean_t inst_branch(unsigned ins);
#ifndef inst_call
 extern boolean_t inst_call(unsigned ins);
#endif
#ifndef inst_return
 extern boolean_t inst_return(unsigned ins);
#endif
#ifndef inst_unconditional_flow_transfer
 extern boolean_t inst_unconditional_flow_transfer(unsigned ins);
#endif
extern unsigned inst_load(unsigned ins);
extern unsigned inst_store(unsigned ins);
extern boolean_t inst_delayed(unsigned ins);
extern db_addr_t
	next_instr_address(db_addr_t pc, unsigned delay_slot, task_t task);
extern unsigned branch_taken(
    unsigned inst,
    unsigned pc,
    db_expr_t (*func)(unsigned int, db_regs_t *),
    db_regs_t *func_data);
extern db_expr_t getreg_val(unsigned regno, db_regs_t *frame);

/*
 * Prototypes from "luna88k/db_trace.c"
 */
extern int frame_is_sane(db_regs_t *regs);
extern char *m88k_exception_name(unsigned int vector);
