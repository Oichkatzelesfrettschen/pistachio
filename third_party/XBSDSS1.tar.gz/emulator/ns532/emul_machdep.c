...
struct emul_regs {
	...
	int	r4;
	int	r3;
	struct emul_regs_2 *usp; /* pointer to rest of user registers */
};
...
#if 0
#ifdef	COMPAT_43
int	ns532_osigvec();		/* forward */
struct sysent ns532_syscall_osigvec =
	{ E_CHANGE_REGS, ns532_osigvec };
#endif	/* COMPAT_43 */
int	ns532_sigaction(), ns532_signal() ;	/* forward */
struct sysent ns532_syscall_sigaction =
        { E_CHANGE_REGS, ns532_sigaction };
struct sysent ns532_syscall_signal = 
	{ E_CHANGE_REGS, ns532_signal };
#endif 0
...
#if 0
#ifdef	COMPAT_43
	sysent[SYS_osigvec] = ns532_syscall_osigvec;
#endif	/* COMPAT_43 */
	sysent[SYS_signal] = ns532_syscall_signal;
	sysent[SYS_sigaction] = ns532_syscall_sigaction;
#endif 0
}
	...
	register struct emul_regs *regs;
{
	...
	fp->sf_r0 = save_regs2.r0;

	fp->sf_r1 = save_regs2.r1;
	fp->sf_r2 = save_regs2.r2;
	...
	fp->sf_sc.sc_pc = save_regs2.pc;
	fp->sf_sc.sc_ps = save_regs2.psr;
	fp->sf_sc.sc_sp = (int)(regs->usp /* + 1 */); /* XXX jvh added +1 Needed? */
	...
	*regs2 = save_regs2;
	regs->usp = regs2;
	regs->fp = (int)fp;
	regs2->pc = USRSTACK-TRAMPOLINE_MAX_SIZE;
}
	...
	struct emul_regs	*regs;
{
	 ...
	 * Change registers.
	 */
	regs2 = ((struct emul_regs_2 *)fp.sf_sc.sc_sp) /* - 1 */; /* XXX jvh added -1 Needed? */

	regs->usp = regs2;
	regs->fp = fp.sf_sc.sc_fp;
	regs2->r0 = fp.sf_r0;
	regs2->r1 = fp.sf_r1;
	regs2->r2 = fp.sf_r2;
	regs2->pc = fp.sf_sc.sc_pc;
	regs2->psr = fp.sf_sc.sc_ps;
 ...
 */
int
ns532_osigvec(serv_port, interrupt, argp, rval, regs)
	mach_port_t		serv_port;
	boolean_t		*interrupt;
	register int		*argp;
	int			*rval;
...
int
ns532_signal(serv_port, interrupt, argp, rval, regs)
	mach_port_t		serv_port;
	boolean_t		*interrupt;
	register int		*argp;
	int			*rval;
...
int
ns532_sigaction(serv_port, interrupt, argp, rval, regs)
	mach_port_t		serv_port;
	boolean_t		*interrupt;
	register int		*argp;
	int			*rval;
...
#define	PSR_ALLCC	(PSR_N | PSR_Z | PSR_F | PSR_V | PSR_L | PSR_C)

	if ((regs2->psr & PSR_ALLCC) == PSR_ALLCC) {
	    new_args[2] = regs2->r1;	/* options */
	    new_args[3] = regs2->r2;	/* rusage_p */
	}
	else {
	    new_args[2] = 0;
	    new_args[3] = 0;
	...
	struct emul_regs	*regs;
{
	...
	struct ns532_thread_state	child_regs;

	
	extern int	child_fork(), get_mod_reg();
	...
	child_regs.psr = regs2->psr;
	child_regs.r3 = regs->r3;
	child_regs.r4 = regs->r4;
	child_regs.r5 = regs->r5;
	child_regs.r6 = regs->r6;
	child_regs.r7 = regs->r7;
	child_regs.fp = regs->fp;
	child_regs.sb = regs->sb;
	child_regs.sp = (int)regs->usp;
	child_regs.mod = get_mod_reg();
